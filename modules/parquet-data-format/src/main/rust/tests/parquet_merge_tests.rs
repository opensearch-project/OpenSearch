...
fn test_process_parquet_files_empty_input() {
    ...
    let result = process_parquet_files(&[], output_path.to_str().unwrap(), "test_index");
...
fn test_process_parquet_files_nonexistent_file() {
    ...
    let result = process_parquet_files(&["/nonexistent/file.parquet".to_string()], output_path.to_str().unwrap(), "test_index");
    assert!(result.is_err());
}
...
fn test_process_single_file() {
    ...
    process_parquet_files(&[input_path.clone()], output_path.to_str().unwrap(), "test_index").unwrap();
...
fn test_merge_files_with_complete_data_verification() {
    ...
    process_parquet_files(&[input1, input2], output_path.to_str().unwrap(), "test_index").unwrap();
