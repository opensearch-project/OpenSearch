...
/// // Create a rate-limited writer with 10 MB/s limit
/// let file = File::create("output.dat")?;
/// let mut writer = RateLimitedWriter::new(file, 10.0)?;
///
/// // Write data - will be automatically rate limited
/// writer.write_all(b"Hello, world!")?;
/// writer.flush()?;
///
/// // Update the rate limit dynamically
/// writer.set_mb_per_sec(20.0)?;
/// # Ok(())
/// # }
/// ```
///
...
impl<W: Write> RateLimitedWriter<W> {
    ...
    /// # fn main() -> std::io::Result<()> {
    /// let file = File::create("output.dat")?;
    /// let writer = RateLimitedWriter::new(file, 10.0)?; // 10 MB/s
    /// # Ok(())
    /// # }
    /// ```
    ///
    ...
    /// # use std::io::Write;
    /// # fn main() -> std::io::Result<()> {
    /// # let file = std::fs::File::create("output.dat")?;
    /// let mut writer = RateLimitedWriter::new(file, 10.0)?;
    ///
    /// // Later, increase the rate limit
    /// writer.set_mb_per_sec(20.0)?;
    ///
    /// // Or disable rate limiting entirely
    /// writer.set_mb_per_sec(0.0)?;
    /// # Ok(())
    /// # }
    /// ```
    ///
