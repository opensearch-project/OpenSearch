...
// rayon for parallel column encoding
use rayon::prelude::*;
...
impl MergeTimings {
    ...
    fn print_summary(&self, total_elapsed: std::time::Duration) {
        ...
        println!();
        println!("--- WRITE (Parallel Column Encoding via rayon) ---");
...
/// Read the parquet schema from the first input file, strip any existing
/// ___row_id column, then append a new INT64 ___row_id column.
/// Returns (parquet root TypePtr, arrow Schema) for use by the writer.
fn build_output_schemas(
    first_input_path: &str,
    first_batch: &DataFrame,
) -> PolarsResult<(Arc<Type>, Arc<ArrowSchema>)> {
    // ── Parquet schema from the input file directly ─────────────────────
    ...
    // ── Arrow schema (for FFI conversion + ArrowRowGroupWriterFactory) ──
...
// =============================================================================
// STREAMING K-WAY MERGE (with parallel column encoding)
// =============================================================================
...
pub fn merge_streaming_with_config(
    ...
    // Find the first active cursor to get the schema source
    ...
    // Parquet schema from the input file (native, no conversion needed)
    // + Arrow schema via FFI from the DataFrame
    ...
    // ── Flush helper (parallel column encoding per row group) ───────────
    macro_rules! flush {
        () => {
            if !output_chunks.is_empty() {
                ...
                // 7. Encode ALL columns in parallel using rayon
