...
public final class AllowlistLoader {
     ...
     * See {@link #loadFromResourceFiles(Class, Map, String...)} for information on how to structure a allowlist
     * text file.
     */
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    // Expects the following format: '}' '\n'
                        ...
                        if ("class".equals(parseType)) {
                            allowlistClasses.add(new AllowlistClass(allowlistClassOrigin, javaClassName,
                                allowlistConstructors, allowlistMethods, allowlistFields, classAnnotations));
                    ...
                    // Expects the following format: ID ID '(' ( ID ( ',' ID )* )? ')' ( 'from_class' | 'bound_to' ) ID annotations? '\n'
                        ...
                        if ("from_class".equals(staticImportType)) {
                            allowlistStatics.add(new AllowlistMethod(origin, targetJavaClassName,
                                    methodName, returnCanonicalTypeName, Arrays.asList(canonicalTypeNameParameters),
                                    annotations));
                        } else if ("bound_to".equals(staticImportType)) {
                            allowlistClassBindings.add(new AllowlistClassBinding(origin, targetJavaClassName,
                                    methodName, returnCanonicalTypeName, Arrays.asList(canonicalTypeNameParameters),
                                    annotations));
                        } else {
                    ...
                    // Handle class definition types.
                        ...
                        if (line.startsWith("(")) {
                            ...
                            annotations = annotationIndex == -1 ?
                                    Collections.emptyList() : parseAllowlistAnnotations(parsers, line.substring(annotationIndex));

                            allowlistConstructors.add(new AllowlistConstructor(
                        ...
                        // Expects the following format: ID ID? ID '(' ( ID ( ',' ID )* )? ')' annotations? '\n'
                            ...
                            allowlistMethods.add(new AllowlistMethod(origin, javaAugmentedClassName, methodName,
    ...
    private static List<Object> parseAllowlistAnnotations(
        Map<String, AllowlistAnnotationParser> parsers, String line) {
