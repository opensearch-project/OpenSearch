...
Otherwise run `bin/opensearch-plugin` as the user that owns all of the Elasticsearch
files.
=====================
...
[source,shell]
-----------------------------------
sudo ES_JAVA_OPTS="-Djavax.net.ssl.trustStore=/path/to/trustStore.jks" bin/opensearch-plugin install https://host/plugin.zip
-----------------------------------
--
...
[source,sh]
---------------------
sudo ES_PATH_CONF=/path/to/conf/dir bin/opensearch-plugin install <plugin name>
---------------------
