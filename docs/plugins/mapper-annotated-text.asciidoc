...
The opensearch markup allows one or more additional tokens to be injected, unchanged, into the token
...
PUT my-index-000001/_doc/1
{
  "my_unstructured_text_field": "[OpenSearch](%40opensearchproject) project",
  "my_twitter_handles": ["@opensearchproject"] <1>
}
...
GET my-index-000001/_search
{
  "query": {
    "query_string": {
        "query": "opensearch OR opensearch\-dashboards",<2>
...
<2> In this example we search for documents that talk about components of the opensearch project
<3> We use the `my_twitter_handles` field here to discover people who are significantly
associated with the opensearch project.
...
* Can have punctuation or numbers e.g. `Apple Inc.` or `@opensearchproject`
...
company `<keyword>`. In this case, a search on the annotated text field for the token `<keyword>`
may match a text document such as this:

    they fired an <keyword> band
...
they don't name clash with text tokens e.g.
    ...
    [<keyword>](Company_<keyword>) released version 7.0 of the <keyword> today
