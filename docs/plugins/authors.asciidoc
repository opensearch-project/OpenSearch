...
:plugin-properties-files: {OpenSearch-root}/buildSrc/src/main/resources

The OpenSearch repository contains examples of:
...
Either fill in this template yourself or, if you are using OpenSearch's Gradle build system, you
...
|`opensearch.version`    |String  | version of OpenSearch compiled against.
...
You will have to release a new version of the plugin for each new OpenSearch release.
This version is checked when the plugin is loaded so OpenSearch will refuse to start
in the presence of plugins with the incorrect `opensearch.version`.
...
`plugins/` directory.  Use `bin/opensearch-plugin install file:///path/to/your/plugin`
...
If you are using the OpenSearch Gradle build system, place this file in
...
[source,java]
--------------------------------------------------
// OpenSearch permission you should check before doPrivileged() blocks
