...
The ingest plugins extend {opensearch} by providing additional ingest node capabilities.
...
The ingest attachment plugin lets {opensearch} extract file attachments in common formats (such as PPT, XLS, and PDF) by
...
{opensearch}. See {ref}/geoip-processor.html[GeoIP processor] for more
details.
...
distributed by default with {opensearch}. See
