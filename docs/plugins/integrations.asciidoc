...
Integrations are not plugins, but are external tools or modules that make it easier to work with OpenSearch.
...
* https://www.zabbix.com/integrations/elasticsearch[Zabbix monitoring template]:
  Monitor the performance and status of your {opensearch} nodes and cluster with Zabbix
