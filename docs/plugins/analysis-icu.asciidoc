...
The ICU Analysis plugin integrates the Lucene ICU module into {opensearch},
...
followed by a colon, then a rule file name. Rule files are placed `OPENSEARCH_HOME/config` directory.

As a demonstration of how the rule files can be used, save the following user file to `$OPENSEARCH_HOME/config/KeywordTokenizer.rbbi`:
