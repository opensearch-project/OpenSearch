...
The `discovery-ec2` plugin allows {opensearch} to find the master-eligible nodes in a
...
To enable EC2 discovery, configure {opensearch} to use the `ec2` seed hosts provider:
...
must be stored in the {ref}/secure-settings.html[{opensearch} keystore]. For example,
...
[source,sh]
----
bin/opensearch-keystore add discovery.ec2.access_key
bin/opensearch-keystore add discovery.ec2.secret_key
----
...
`discovery.ec2.access_key` ({ref}/secure-settings.html[Secure], {ref}/secure-settings.html#reloadable-secure-settings[reloadable])::
    ...
    is sensitive and must be stored in the {opensearch} keystore.
...
`discovery.ec2.secret_key` ({ref}/secure-settings.html[Secure], {ref}/secure-settings.html#reloadable-secure-settings[reloadable])::
    ...
    This setting is sensitive and must be stored in the {opensearch} keystore.
...
`discovery.ec2.session_token` ({ref}/secure-settings.html[Secure], {ref}/secure-settings.html#reloadable-secure-settings[reloadable])::
    ...
    stored in the {opensearch} keystore.
...
`opensearch-host-name` and set `host_type: tag:opensearch-host-name` then
the `discovery-ec2` plugin will read each instance's host name from the value
of the `opensearch-host-name` tag.
...
EC2. By default each {opensearch} node only binds to `localhost`, which will prevent it
...
{opensearch} cluster on your own AWS infrastructure.
...
Store] is recommended for {opensearch} clusters as it offers excellent performance and
is cheaper than EBS-based storage. {opensearch} is designed to work well with this kind
of ephemeral storage because it replicates each shard across multiple nodes. If
a node fails and its Instance Store is lost then {opensearch} will rebuild any lost
...
precisely the behaviour that {opensearch} requires of its filesystem, and this has been
...
achieve the same benefits using {opensearch} directly.
...
* Do not span a cluster across regions. {opensearch} expects that node-to-node
