...
Even if Hadoop is already installed on the OpenSearch nodes, for security reasons, the required libraries need to be placed under the plugin folder. Note that in most cases, if the distro is compatible, one simply needs to configure the repository with the appropriate Hadoop configuration files (see below).
...
On each OpenSearch node, place the appropriate keytab file in the node's configuration location under the
...
`opensearch/hostname@REALM`) then each node will need its own unique keytab file for the principal assigned to that
host!
...
name. OpenSearch will automatically replace the pattern with the hostname of the node at runtime:
...
[discrete]
===== Authorization
Once OpenSearch is connected and authenticated to HDFS, HDFS will infer a username to use for
...
like `opensearch@REALM` or `opensearch/hostname@REALM` then the username that HDFS
extracts for file access checks will be `opensearch`.

NOTE: The repository plugin makes no assumptions of what OpenSearch's principal name is. The main fragment of the
Kerberos principal is not required to be `opensearch`. If you have a principal or service name that works better
