...
* Your project ID, e.g. `opensearch-cloud`. Get it from https://code.google.com/apis/console/[Google API Console].
...
[source,sh]
--------------------------------------------------
gcloud config set project opensearch-cloud
--------------------------------------------------
...
[source,text]
--------------------------------------------------
Created [https://www.googleapis.com/compute/v1/projects/opensearch-cloud-1070/zones/us-central1-f/instances/myesnode1].
...
Failing to set this will result in unauthorized messages when starting OpenSearch.
See <<discovery-gce-usage-tips-permissions>>.
==============================================

Once connected, {stack-gs}/get-started-elastic-stack.html#install-elasticsearch[install {opensearch}]:

[[discovery-gce-usage-long-install-plugin]]
===== Install OpenSearch discovery gce plugin
...
sudo bin/opensearch-plugin install discovery-gce
--------------------------------------------------

Open the `opensearch.yml` file:
...
[source,sh]
--------------------------------------------------
sudo /etc/init.d/opensearch start
--------------------------------------------------
...
[source,sh]
--------------------------------------------------
sudo vi /etc/opensearch/log4j2.properties
--------------------------------------------------
...
gcloud compute images create elasticsearch-2-0-0 --source-uri gs://opensearchimage/e4686d7f5bf904a924ae0cfeb58d0827c6d5b966.image.tar.gz
...
[source,sh]
--------------------------------------------------
# Just change node name (here myopensearchnode2)
...
instances that _are_ running the master-eligible {opensearch} nodes, and then filtering
...
[source,sh]
--------------------------------------------------
gcloud compute instances create myesnode1 --project=opensearch-cloud \
       --scopes=compute-rw \
       --tags=elasticsearch,dev
--------------------------------------------------

Then, define it in `opensearch.yml`:
...
By default, OpenSearch GCE plugin assumes that you run OpenSearch on 9300 default port.
But you can specify the port value OpenSearch is meant to use using google compute engine metadata `opensearch_port`:

[[discovery-gce-usage-port-create]]
===== When creating instance

Add `--metadata opensearch_port=9301` option:
...
[source,sh]
--------------------------------------------------
mvn -Dtests.gce=true -Dtests.config=/path/to/config/file/opensearch.yml clean test
--------------------------------------------------
