...
However, that strategy is **not** supported for use with {opensearch}. The
...
To provide this file to the plugin, it must be stored in the {ref}/secure-settings.html[{opensearch} keystore].  You must
...
inside `opensearch.yml`. The default client name looked up by a `gcs` repository is
...
{ref}/secure-settings.html[{opensearch} keystore]. This is the case for the service account file:

[source,sh]
----
bin/opensearch-keystore add-file gcs.client.default.credentials_file /path/service-account.json
----
