...
* the <<dynamic-mapping,data type>> detected by OpenSearch, with <<match-mapping-type,`match_mapping_type`>>.
...
By default OpenSearch will map string fields as a `text` field with a sub
`keyword` field. However if you are only indexing structured content and not
interested in full text search, you can make OpenSearch map your fields
...
OpenSearch to map it only as a text field:
...
When doing time series analysis with OpenSearch, it is common to have many
