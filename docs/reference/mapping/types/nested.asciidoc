...
OpenSearch has no concept of inner objects. Therefore, it flattens object
...
nested object. Because of the expense associated with `nested` mappings, OpenSearch puts
settings in place to guard against performance problems:

include::{opensearch-repo-dir}/mapping.asciidoc[tag=nested-fields-limit]

In the previous example, the `user` mapping would count as only 1 towards this limit.

include::{opensearch-repo-dir}/mapping.asciidoc[tag=nested-objects-limit]
