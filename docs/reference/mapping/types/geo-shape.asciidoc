...
`precision` parameter instead. However, OpenSearch only uses the
...
in OpenSearch is 24; the default is 9.
...
number of levels for the quad trees in OpenSearch is 29; the default is 21.
...
The defaults in OpenSearch for both implementations are a compromise
...
|`Point` |`POINT` |`point` |A single geographic coordinate. Note: OpenSearch uses WGS-84 coordinates only.
...
In GeoJSON and WKT, and therefore OpenSearch, the correct *coordinate
...
OpenSearch accepts both clockwise and counterclockwise polygons if they
...
The following provides an example of an ambiguous polygon.  OpenSearch will
...
OpenSearch supports an `envelope` type, which consists of coordinates
...
OpenSearch supports a `circle` type, which consists of a center
