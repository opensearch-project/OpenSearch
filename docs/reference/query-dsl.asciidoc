...
OpenSearch provides a full Query DSL (Domain Specific Language) based on JSON to define queries.
...
** <<query-dsl-prefix-query,`prefix queries`>>  (except on fields without <<index-prefixes, `index_prefixes`>>)
