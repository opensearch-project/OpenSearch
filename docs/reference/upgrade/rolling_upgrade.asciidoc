...
A rolling upgrade allows an {opensearch} cluster to be upgraded one node at
a time so upgrading does not interrupt service. Running multiple versions of
{opensearch} in the same cluster beyond the duration of an upgrade is
...
. *Upgrade any plugins.*
+
Use the `opensearch-plugin` script to install the upgraded version of each
installed {opensearch} plugin. All plugins must be upgraded when you upgrade
a node.
