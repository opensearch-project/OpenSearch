...
{kib} features. Therefore, before you can use {kib} features in {version}, 
you must ensure the internal indices have a compatible index structure.
...
This enables you to reindex indices that reside on clusters running any version of {opensearch}.
...
{opensearch} provides backwards compatibility support that enables
...
{opensearch} does not support forward compatibility across major versions.
...
`reindex.remote.whitelist` in `opensearch.yml`.
+
--
[source,yaml]
--------------------------------------------------
