...
{opensearch} sometimes needs to replay some of the operations that were performed on a
...
At the Lucene level there are really only two write operations that {opensearch}
...
true of document deletions. To solve this, {opensearch} uses a feature called _soft
deletes_ to preserve recent deletions in the Lucene index so that they can be
replayed.

{opensearch} only preserves certain recently-deleted documents in the index because a
soft-deleted document still takes up some space. Eventually {opensearch} will fully
discard these soft-deleted documents to free up that space so that the index
does not grow larger and larger over time. Fortunately {opensearch} does not need to be
...
operations, so {opensearch} tries to retain all of the operations it expects to need to
replay in future.

{opensearch} keeps track of the operations it expects to need to replay in future using
...
for itself. For example, this shard copy might be a replica of a shard. 
Each retention lease
...
it will not need to replay those operations in future. {opensearch} discards
...
which means that {opensearch} will preserve all new operations so they can be replayed
...
its retention lease may expire. This protects {opensearch} from retaining history
forever if a shard copy fails permanently, because once a retention lease has
expired {opensearch} can start to discard history again. If a shard copy recovers after
its retention lease has expired then {opensearch} will fall back to copying the whole
...
include::{opensearch-ref-dir}/index-modules.asciidoc[tag=index-soft-deletes-tag]

include::{opensearch-ref-dir}/index-modules.asciidoc[tag=index-soft-deletes-retention-tag]
