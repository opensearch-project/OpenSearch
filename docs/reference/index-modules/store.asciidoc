...
OpenSearch will pick the best implementation based on the operating
environment.

The storage type can also be explicitly set for all indices by configuring the
store type in the `config/opensearch.yml` file:
...
By default, OpenSearch completely relies on the operating system file system
...
extensions to look at since OpenSearch performs random access on them.
