...
When creating a new index in OpenSearch it is possible to configure how the Segments
...
`index.sort.mode`::
    ...
    OpenSearch supports sorting by multi-valued fields.
...
By default in OpenSearch a search request must visit every document that match a query to
...
OpenSearch will detect that the top docs of each segment are already sorted in the index
...
This time, OpenSearch will not try to count the number of documents and will be able to terminate the query
