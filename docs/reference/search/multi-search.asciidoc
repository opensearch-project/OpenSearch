...
`index` target.
+
Wildcard (`*`) expressions are supported. To search all indices
...
Indices and index aliases to search. Wildcard (`*`) expressions
...
The header part includes which indices, and index aliases to
...
The endpoint also allows you to search against indices and index
