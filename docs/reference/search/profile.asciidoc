...
equivalent name in OpenSearch.  The `description` field displays the Lucene 
...
sorting" `Collector` used by {opensearch}. The `reason` field attempts to give a plain 
...
`SimpleTopScoreDocCollector` is used by {opensearch} to detect if the current search was 
...
`aggregation`::
    ...
    A collector that OpenSearch uses to run aggregations against the query scope.  A single `aggregation`
