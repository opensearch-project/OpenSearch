...
searches do not support the full {opensearch} <<query-dsl,Query DSL>> but are handy for
testing.
+
...
--
(integer)
Milliseconds it took {opensearch} to execute the request.
...
* Time needed to send the request to {opensearch}
