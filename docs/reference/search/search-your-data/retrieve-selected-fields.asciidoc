...
make use of {opensearch}'s index structures or related optimizations. This can sometimes
...
The `fields` parameter handles field types like <<alias, field aliases>> 
whose values aren't always present in
...
even when there is a single value in the `_source`. This is because {opensearch} has
...
field is stored separately, {opensearch} only reads the field values that were requested
