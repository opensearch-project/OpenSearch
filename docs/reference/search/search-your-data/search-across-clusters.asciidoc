...
If `cluster_two` is disconnected or unavailable during a {ccs}, {opensearch} won't
...
By default, {opensearch} reduces the number of network roundtrips between remote
clusters. This reduces the impact of network delays on search speed. However,
{opensearch} can't reduce network roundtrips for large search requests, such as those
...
requests that include a scroll or inner hits, {opensearch} sends multiple outgoing and
...
For example, a coordinating node running {opensearch} 5.6 can search a remote cluster
running {opensearch} 6.8, but that cluster can not be upgraded to 7.1. In this case
you should first upgrade the coordinating node to 7.1 and then upgrade remote
cluster.

WARNING: Running multiple versions of {opensearch} in the same cluster beyond the
