...
To protect against hardware failure and increase search capacity, {opensearch} can store
copies of an index's data across multiple shards on multiple nodes. When running
a search request, {opensearch} selects a node containing a copy of the index's data and
...
By default, {opensearch} uses _adaptive replica selection_ to route search requests.
...
<<cluster-update-settings,cluster settings API>>. If disabled, {opensearch} routes
...
node can also reject a search request, which {opensearch} would re-route to another
node.
...
By default, {opensearch} doesn't reject search requests based on the number of shards
