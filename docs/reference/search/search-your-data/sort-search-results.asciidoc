...
{opensearch} supports sorting by array or multi-valued fields. The `mode` option
...
and can be useful for searches across multiple indices where the sort field is mapped differently.
...
{opensearch} also supports sorting by
...
The nested `path` needs to be specified; otherwise, {opensearch} doesn't know on what nested level sort values need to be captured.
...
The `nested_path` needs to be specified at each level; otherwise, {opensearch} doesn't know on what nested level sort values need to be captured.
...
then {opensearch} will handle it as if there was a mapping of type
