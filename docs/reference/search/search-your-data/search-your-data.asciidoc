...
You can think of a query as a question, written in a way {opensearch} understands.
...
A _search_ consists of one or more queries that are combined and sent to {opensearch}.
...
<<search-aggregations,aggregate>> data stored in {opensearch} indices.
...
several indices in the same request. You can even boost search
...
*Run an async search* +
{opensearch} searches are designed to run on large volumes of data quickly, often
...
You can use the `timeout` parameter to specify a duration you'd
...
API>>. {opensearch} also automatically cancels a search request when your client's HTTP
