...
=== Index APIs
{opensearch} index APIs are now documented in <<indices>>.
...
[role="exclude",id="administer-opensearch"]
=== Administering {opensearch}
...
({es-issue}55257[#55257]) from the {opensearch} Reference
...
for providing general testing guidance for applications that communicate with {opensearch}.
...
This page was deleted.
Information about the Java testing framework was removed
({es-issue}55257[55257]) from the {opensearch} Reference because it was out of date and
...
guidance for applications that communicate with {opensearch}.
...
=== Using the {opensearch} test classes
...
See <<settings,Configuring OpenSearch>> for settings information:
...
See <<add-opensearch-nodes>>.

[role="exclude",id="_installation"]
=== Installation

See <<install-opensearch>>.
