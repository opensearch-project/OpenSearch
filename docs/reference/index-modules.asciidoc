...
+
NOTE: The number of shards are limited to `1024` per index. This limitation is a safety limit to prevent accidental creation of indices that can destabilize a cluster due to resource allocation. The limit can be modified by specifying `export OPENSEARCH_JAVA_OPTS="-Des.index.max_number_of_shards=128"` system property on every node that is part of the cluster.
...
deprecated:[7.6.0, Creating indices with soft-deletes disabled is deprecated and will be removed in future OpenSearch versions.]
...
{opensearch} 6.5.0. Defaults to `true`.
