...
Wherever scripting is supported in the OpenSearch API, the syntax follows
the same pattern:
...
The first time OpenSearch sees a new script, it compiles it and stores the
...
OpenSearch will reject the new dynamic scripts with a
...
Scripts can't make use of {opensearch}'s index structures or related optimizations. This
...
=== Script errors
OpenSearch returns error details when there is a compliation or runtime
