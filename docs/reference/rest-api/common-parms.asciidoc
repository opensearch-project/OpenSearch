...
tag::generation[]
Generation number, such as `0`. {opensearch} increments this generation number
...
such as `1264`.
+
A value of `-1` indicates {opensearch} was unable to compute this number.
end::memory[]
...
tag::refresh[]
`refresh`::
(Optional, enum) If `true`, {opensearch} refreshes the affected shards to make this
