...
https://lucene.apache.org/core/[Lucene], which {opensearch} uses internally to power
indexing and search. As a result, all document counts include hidden
<<nested,nested>> documents.

To get an accurate count of {opensearch} documents, use the <<cat-count,cat count>> or
...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=bytes]
...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=master-timeout]
...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=time]
