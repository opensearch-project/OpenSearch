...
By default, {opensearch} uses the <<mapping-date-format,date `format`>> provided in the
...
range>> aggregations, {opensearch} replaces missing date components with the following
...
For example, if the format is `yyyy-MM`, {opensearch} converts a `gt` value of `2099-12`
...
{opensearch} rounds <<date-math,date math>> values in parameters as follows:
...
<2> With a UTC offset of `+01:00`, {opensearch} converts this date to
