...
By default, {opensearch} limits the `terms` query to a maximum of 65,536
...
<<highlighting,Highlighting>> is best-effort only. {opensearch} may not
...
===== Terms lookup
Terms lookup fetches the field values of an existing document. {opensearch} then uses
...
+
--
(Required, string) Name of the field from which to fetch field values. {opensearch} uses
...
field, {opensearch} returns both documents.
