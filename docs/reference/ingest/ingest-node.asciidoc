...
is possible because OpenSearch doesn't allow fields in the source that have the
...
document IDs, you cannot use the `{{_id}}` value in an ingest processor. {opensearch}
...
under the `_ingest.timestamp` key of the ingest metadata. The ingest timestamp is the time when OpenSearch
...
Unlike OpenSearch metadata fields, the ingest metadata field name `_ingest` can be used as a valid field name
...
`script.painless.regex.enabled: true` must be set in `opensearch.yml` to use regular
...
`plugin.mandatory` setting to the `config/opensearch.yml` file, for example:
...
Additional ingest processors can be implemented and installed as OpenSearch {plugins}/intro.html[plugins].
