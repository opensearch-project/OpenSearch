...
We might want to see which tags are strongly associated with `#opensearch` on StackOverflow
forum posts but ignoring the effects of some prolific users with a tendency to misspell #opensearch-dashboards as #dashboards.
...
Please note that OpenSearch will ignore the choice of execution hint if it is not applicable and that there is no backward compatibility guarantee on these hints.
