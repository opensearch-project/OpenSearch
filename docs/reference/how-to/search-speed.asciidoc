...
OpenSearch heavily relies on the filesystem cache in order to make search
fast. In general, you should make sure that at least half the available memory
goes to the filesystem cache so that OpenSearch can keep hot regions of the
...
that are heavily used for bucketing aggregations, you can tell {opensearch} to construct
...
If the machine running OpenSearch is restarted, the filesystem cache will be
...
cache to each shard, and the filesystem cache is probably OpenSearch's
...
performance result and reduced load.
