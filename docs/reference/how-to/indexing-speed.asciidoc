...
=== Use multiple workers/threads to send data to OpenSearch

A single thread sending bulk requests is unlikely to be able to max out the
indexing capacity of an OpenSearch cluster. In order to use all resources
...
OpenSearch tells you that it cannot keep up with the current indexing rate.
...
include::{opensearch-repo-dir}/indices/refresh.asciidoc[tag=refresh-interval-default]
...
default behavior means that OpenSearch will refresh your index every 1
...
OpenSearch, it may be beneficial to set `index.number_of_replicas` to `0` in
...
make sure to give at least half the memory of the machine running OpenSearch
to the filesystem cache.

[discrete]
=== Use auto-generated ids

When indexing a document that has an explicit id, OpenSearch needs to check
...
auto-generated ids, OpenSearch can skip this check, which makes indexing
faster.
...
storage works very well with OpenSearch, and it is appealing since it is so
...
indexing performance does not typically improve). OpenSearch takes that
