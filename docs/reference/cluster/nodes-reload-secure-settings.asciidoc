...
When the {opensearch} keystore is password protected and not simply obfuscated, you must
...
the node-specific {opensearch} keystore password.
...
NOTE: {opensearch} requires consistent secure settings across the cluster nodes, but
...
`secure_settings_password`::
  (Optional, string) The password for the {opensearch} keystore.

[[cluster-nodes-reload-secure-settings-api-example]]
==== {api-examples-title}

The following examples assume a common password for the {opensearch} keystore on every
node of the cluster:
