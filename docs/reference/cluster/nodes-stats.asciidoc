...
cgroup as the {opensearch} process should have their access to CPU resources
reallocated.

`cfs_quota_micros` (Linux only)::
(integer)
The total amount of time (in microseconds) for which all tasks in
the same cgroup as the {opensearch} process can run during one period
`cfs_period_micros`.
...
(integer)
The number of times all tasks in the same cgroup as the {opensearch} process have
been throttled.
...
tasks in the same cgroup as the {opensearch} process. This value can be too big to
...
tasks in the same cgroup as the {opensearch} process. This value is stored as a
...
space the {opensearch} node can utilise.
...
`operations` (Linux only)::
    ...
    {opensearch} completed since starting {opensearch}.

`read_operations` (Linux only)::
    (integer)
    The total number of read operations for across all devices used by {opensearch}
    completed since starting {opensearch}.

`write_operations` (Linux only)::
    (integer)
    The total number of write operations across all devices used by {opensearch}
    completed since starting {opensearch}.

`read_kilobytes` (Linux only)::
    (integer)
    The total number of kilobytes read across all devices used by {opensearch} since
    starting {opensearch}.

`write_kilobytes` (Linux only)::
    (integer)
    The total number of kilobytes written across all devices used by {opensearch} since
    starting {opensearch}.
=======
======
