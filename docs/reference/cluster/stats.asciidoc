...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=node-filter]
...
`status`::
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=cluster-health-status]
+
See <<cluster-health>>.
...
selected nodes.
+
This number is based on documents in Lucene segments. {opensearch} reclaims the disk
...
`nodes.fs.free`. This is the actual amount of free disk space the selected {opensearch}
nodes can use.
...
`name`:::
(string)
Name of the {opensearch} plugin.

`version`:::
(string)
{opensearch} version for which the plugin was built.
...
`flavor`:::
(string)
Type of {opensearch} distribution, such as `default` or `OSS`, used by one or more
selected nodes.
      ...
      "plugins": [
        {
          "name": "analysis-icu",
          "version": "{version}",
          "description": "The ICU Analysis plugin integrates Lucene ICU module into opensearch, adding ICU relates analysis components.",
