...
* `name`: plugin name
* `version`: version of OpenSearch the plugin was built for
...
`settings`::
Lists all node settings in use as defined in the `opensearch.yml` file.
...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=node-id]
...
`version`::
		{opensearch} version running on this node.
...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=timeoutparms]
...
// TESTRESPONSE[s/"opensearch"/$body.cluster_name/]
...
// TESTRESPONSE[s/"host": "node-0.opensearch.org"/"host": $body.$_path/]
