...
* `elasticsearch.yml` for configuring OpenSearch
...
[source,sh]
-------------------------------
OPENSEARCH_PATH_CONF=/path/to/my/config ./bin/elasticsearch
-------------------------------
...
`OPENSEARCH_PATH_CONF=/etc/elasticsearch` entry in one of these files accordingly to
