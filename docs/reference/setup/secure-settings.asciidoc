...
keystore and the <<elasticsearch-keystore,`opensearch-keystore` tool>> to
...
All the modifications to the keystore take effect only after restarting {opensearch}.

These settings, just like the regular ones in the `opensearch.yml` config file,
...
Just like the settings values in `opensearch.yml`, changes to the keystore
...
using the `bin/opensearch-keystore add` command, call:
...
<1> The password that the {opensearch} keystore is encrypted with.
