...
comes with a `opensearch-service.bat` command which will setup Elasticsearch to run as a
service.
...
Download the `.zip` archive for Elasticsearch v{version} from: https://artifacts.opensearch.org/downloads/opensearch/opensearch-{version}-windows-x86_64.zip
...
Elasticsearch loads its configuration from the `%OPENSEARCH_HOME%\config\opensearch.yml`
