...
comes with a `elasticsearch-service.bat` command which will setup {opensearch} to run as a
service.
...
{opensearch} loads its configuration from the `%ES_HOME%\config\elasticsearch.yml`
