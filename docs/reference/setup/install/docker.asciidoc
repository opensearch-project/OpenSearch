[[docker]]
=== Install {opensearch} with Docker

{opensearch} is also available as Docker images.
...
https://www.docker.opensearch.org[www.docker.opensearch.org]. The source files
are in
...
WARNING: Version {version} of {opensearch} has not yet been released, so no
...
https://www.docker.opensearch.org[www.docker.opensearch.org].
...
WARNING: Version {version} of the {opensearch} Docker image has not yet been released.

endif::[]

ifeval::["{release-state}"!="unreleased"]

To start a single-node {opensearch} cluster for development or testing, specify
...
To get a three-node {opensearch} cluster up and running in Docker,
...
+
--
WARNING: Version {version} of {opensearch} has not yet been released, so a
...
This sample Docker Compose file brings up a three-node {opensearch} cluster.
Node `opensearch01` listens on `localhost:9200` and `opensearch02` and `opensearch03` talk to `opensearch01` over a Docker network.

Please note that this configuration exposes port 9200 on all network interfaces, and given how
Docker manipulates `iptables` on Linux, this means that your {opensearch} cluster is publically accessible,
...
{opensearch} will then only be accessible from the host machine itself.
...
===== Configuration files must be readable by the `opensearch` user

By default, {opensearch} runs inside the container as user `opensearch` using
uid:gid `1000:0`.
...
If you are bind-mounting a local directory or file, it must be readable by the `opensearch` user.
...
uid:gid `1000:0`, which provides the required read/write access to the {opensearch} process.


===== Increase ulimits for nofile and nproc

Increased ulimits for <<setting-system-settings,nofile>> and <<max-number-threads-check,nproc>>
must be available for the {opensearch} containers.
...
file under `/usr/share/opensearch/config/jvm.options.d` that includes your
...
method, you can also configure this by using the `OPENSEARCH_JAVA_OPTS` environment
variable to set the heap size. For example, to use 16 GB, specify
`-e OPENSEARCH_JAVA_OPTS="-Xms16g -Xmx16g"` with `docker run`. Note that while the
default root `jvm.options` file sets a default heap of 1 GB, any value you set
in `OPENSEARCH_JAVA_OPTS` will override it. The `docker-compose.yml` file above sets the heap size to 512 MB.
...
You should use a volume bound on `/usr/share/opensearch/data` for the following reasons:

.  The data of your {opensearch} node won't be lost if the container is killed

. {opensearch} is I/O sensitive and the Docker storage driver is not ideal for fast I/O
...
==== Configuring {opensearch} with Docker

When you run in Docker, the <<config-files-location,{opensearch} configuration files>> are loaded from
...
You can set individual {opensearch} configuration parameters using Docker environment variables.
...
variable name with `_FILE`. This is useful for passing secrets such as passwords to {opensearch}
without specifying them directly.

For example, to set the {opensearch} bootstrap password from a file, you can bind mount the
...
You can also override the default command for the image to pass {opensearch} configuration
parameters as command line options. For example:

[source,sh]
--------------------------------------------
docker run <various parameters> bin/opensearch -Ecluster.name=mynewclustername
--------------------------------------------
...
===== Mounting {opensearch} configuration files

Create custom config files and bind-mount them over the corresponding files in the Docker image.
For example, to bind-mount `custom_opensearch.yml` with `docker run`, specify:

[source,sh]
--------------------------------------------
-v full_path_to/custom_opensearch.yml:/usr/share/opensearch/config/opensearch.yml
--------------------------------------------

IMPORTANT: The container **runs {opensearch} as user `opensearch` using
...
===== Mounting an {opensearch} keystore

By default, {opensearch} will auto-generate a keystore file for secure settings. This
...
`opensearch-keystore` utility to create a password-protected keystore and
bind-mount it to the container as
`/usr/share/opensearch/config/opensearch.keystore`. In order to provide
...
[source, sh]
--------------------------------------------
-v full_path_to/opensearch.keystore:/usr/share/opensearch/config/opensearch.keystore
...
[source,sh,subs="attributes"]
--------------------------------------------
FROM docker.opensearch.org/opensearch/opensearch:{version}
...
[source,sh]
--------------------------------------------
docker build --tag=opensearch-custom .
docker run -ti -v /usr/share/opensearch/data opensearch-custom
--------------------------------------------
