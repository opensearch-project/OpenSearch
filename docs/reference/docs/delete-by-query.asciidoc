...
When you submit a delete by query request, {opensearch} gets a snapshot of the index
...
While processing a delete by query request, {opensearch} performs multiple search
...
<<tasks,`task`>> you can use to cancel or get the status of the task. {opensearch} creates a
