...
The source and destination can be any pre-existing index or index alias. 
However, the source and destination must be different.
...
in the `reindex.remote.whitelist` setting of `opensearch.yml`. See
<<reindex-from-remote>>.
...
`version_type` or setting it to `internal` causes OpenSearch
to blindly dump documents into the destination, overwriting any that happen to have
the same ID.

Setting `version_type` to `external` causes OpenSearch to preserve the
...
{opensearch} creates a record of this task as a document at `.tasks/_doc/${taskId}`.
...
Setting `slices` to `auto` will let OpenSearch choose the number of slices to
...
slices based on the index with the smallest number of shards.
...
`source`::
`index`:::
(Required, string) The name of the index or index alias you are copying _from_.
...
`remote`:::
`host`::::
(Optional, string) The URL for the remote instance of {opensearch} that you want to index _from_.
...
`dest`::
`index`:::
(Required, string) The name of the index or index alias you are copying _to_.
...
The new template for the `metricbeat-*` indices is already loaded into OpenSearch,
...
Reindex supports reindexing from a remote OpenSearch cluster:
...
will connect to the remote OpenSearch node using basic auth. Be sure to use `https` when
...
Remote hosts have to be explicitly allowed in opensearch.yml using the
...
This feature should work with remote clusters of any version of OpenSearch
...
WARNING: {opensearch} does not support forward compatibility across major versions. For
example, you cannot reindex from a 7.x cluster into a 6.x cluster.

To enable queries sent to older versions of OpenSearch the `query` parameter
...
specified in the `opensearch.yml` file, with the exception of the
