...
Internally, OpenSearch has marked the old document as deleted and added an
...
`refresh`::
(Optional, Boolean)
If `true`, {opensearch} refreshes the affected shards to make this operation visible to
...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=stored_fields]
...
{opensearch} returns a status code of `200 - OK` if the document exists, or
