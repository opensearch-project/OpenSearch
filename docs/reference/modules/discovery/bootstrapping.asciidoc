...
Starting an {opensearch} cluster for the very first time requires the initial
...
nodes on the command-line that is used to start {opensearch}:
...
their cluster name when they first connect to each other, and {opensearch}
