...
{opensearch} achieves this robustness by considering each action to have
...
{opensearch} allows you to add and remove master-eligible nodes to a running
...
{opensearch} uses an election process to agree on an elected master node, both
...
nodes and then starting them back up again. By default {opensearch} can remain
