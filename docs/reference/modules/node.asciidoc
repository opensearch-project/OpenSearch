...
Any time that you start an instance of {opensearch}, you are starting a _node_. A
collection of connected nodes is called a <<modules-cluster,cluster>>. If you
are running a single node of {opensearch}, then you have a cluster of one node.
...
remote clusters using <<modules-cross-cluster-search,{ccs}>>.
...
`opensearch.yml` file and restarting it. This is known as _repurposing_ a
...
the <<node-tool-repurpose,`opensearch-node repurpose`>> tool to delete any
...
to `$OPENSEARCH_HOME/data` but can be configured in the `opensearch.yml` config
file an absolute path or a path relative to `$OPENSEARCH_HOME` as follows:

[source,yaml]
----
path.data:  /var/opensearch/data
----
...
[source,sh]
----
./bin/opensearch -Epath.data=/var/opensearch/data
----
...
By default, {opensearch} is configured to prevent more than one node from sharing the
