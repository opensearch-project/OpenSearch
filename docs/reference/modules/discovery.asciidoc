...
<<modules-discovery-quorums>>::
    ...
    How {opensearch} uses a quorum-based voting mechanism to
    make decisions even if some nodes are unavailable.

<<modules-discovery-voting>>::

    How {opensearch} automatically updates voting configurations as nodes leave and join
    a cluster.

<<modules-discovery-bootstrap-cluster>>::

    Bootstrapping a cluster is required when an {opensearch} cluster starts up
...
<<cluster-fault-detection>>::
    ...
    {opensearch} performs health checks to detect and remove faulty nodes.
