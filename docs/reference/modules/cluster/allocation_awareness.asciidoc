...
You can use custom node attributes as _awareness attributes_ to enable {opensearch}
...
By default {opensearch} uses <<search-adaptive-replica,adaptive replica selection>>
...
disabled by specifying `export OPENSEARCH_JAVA_OPTS="$OPENSEARCH_JAVA_OPTS -Des.search.ignore_awareness_attributes=true"`
...
. Tell {opensearch} to take one or more awareness attributes into account when
...
If `rack_two` fails and takes down both its nodes, by default {opensearch}
...
By default, if one location fails, OpenSearch assigns all of the missing
...
in `zone1` and `zone2`, you can use forced awareness to prevent OpenSearch
...
to `zone1` and create an index with 5 shards and 1 replica, OpenSearch creates
