...
For faster responses, {opensearch} caches the results of frequently run aggregations in
...
{opensearch} routes searches with the same preference string to the same shards. If the
...
When running aggregations, {opensearch} uses <<number,`double`>> values to hold and
