...
deprecated::[7.6, "Synced-flush is deprecated. Use <<indices-flush,flush>> instead. A flush has the same effect as a synced flush on OpenSearch 7.6 or later."]
...
{opensearch} will perform the synced flush on a "best-effort" basis: shards that do not
...
{opensearch} keeps track of which shards have received indexing activity recently, and
considers shards that have not received any indexing operations for 5 minutes to
be inactive.

When a shard becomes inactive {opensearch} performs a special kind of flush
...
When allocating shard replicas, {opensearch} must ensure that each replica contains the
same data as the primary. If the shard copies have been synced-flushed and the
replica shares a `sync_id` with the primary then {opensearch} knows that the two copies
...
{opensearch} may trigger an automatic flush of a shard at any time if there are
...
[source,console]
--------------------------------------------------
POST /kimchy,opensearch/_flush/synced
--------------------------------------------------
// TEST[skip: Synced flush can conflict with scheduled flushes in doc tests]
