...
called _generations_, and {opensearch} will delete any generation files once they are no
...
+
To flush all indices in a cluster, omit this parameter or use
`_all` or `*`.
...
[[flush-api-all-ex]]
===== Flush all indices in a cluster
