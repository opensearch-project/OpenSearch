...
===== Why doesn't OpenSearch support incremental resharding?
...
become `active`. At that  point, OpenSearch will try to allocate any
...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=target-index]
...
include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=target-index-aliases]

include::{opensearch-repo-dir}/rest-api/common-parms.asciidoc[tag=target-index-settings]
