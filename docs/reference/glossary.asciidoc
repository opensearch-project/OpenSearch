...
that allows OpenSearch to perform full text queries.
...
// tag::api-key-def[]
A unique identifier that you can use for authentication when submitting {opensearch} requests.
...
A document is a JSON document which is stored in OpenSearch. It is
...
{opensearch} records index and delete operations in the translog and
...
Most {opensearch} APIs accept an index alias
...
In most cases, the index parameter in an {opensearch} request can be the name of a specific index,
...
The cluster that pulls data from a <<glossary-remote-cluster,remote cluster>> in {ccs}.
...
A node is a running instance of OpenSearch which belongs to a
...
A request for information from {opensearch}. You can think of a query as a question,
written in a way {opensearch} understands. A search consists of one or more queries
combined.
+
...
destination can be any pre-existing index or index alias.
...
or upgrade {opensearch} between incompatible versions.
// end::reindex-def[]
--
...
A rollover target can be an <<indices-aliases, index alias>>.
...
which is managed automatically by OpenSearch. An index is a logical
...
+
OpenSearch distributes shards amongst all <<glossary-node,nodes>> in the
...
also mount indices from snapshots as read-only
...
A term is an exact value that is indexed in OpenSearch. The terms
