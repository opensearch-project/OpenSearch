...
* `opensearch-node override-version` enables you to start up a node
  ...
  {opensearch}. This may sometimes allow you to downgrade to an earlier version of
  {opensearch}.
...
Sometimes {opensearch} nodes are temporarily stopped, perhaps because of the need to
...
{opensearch} <<modules-discovery-quorums,requires a response from a majority of the
...
The data that {opensearch} writes to disk is designed to be read by the current version
...
version, causing {opensearch} to believe that it is compatible with the on-disk data.
