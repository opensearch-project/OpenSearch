...
{opensearch} that created the snapshot, the list of included indices, the current state of the
...
{opensearch}. As a result, standard recovery monitoring services can be used
...
primary shards is completed, {opensearch} switches to the standard replication
