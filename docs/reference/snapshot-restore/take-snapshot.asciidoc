...
The snapshot process is incremental. In the process of making the snapshot, {opensearch} analyses
...
The snapshot process starts immediately for the primary shards that have been started and are not relocating at the moment. {opensearch} waits for
...
filtering. {opensearch} can only move a shard to another node (according to the current allocation
