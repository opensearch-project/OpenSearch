...
// tag::snapshot-intro[]
A _snapshot_ is a backup taken from a running {opensearch} cluster.
...
{opensearch} takes snapshots incrementally: the snapshotting process only copies data
...
// tag::backup-warning[]
WARNING: You cannot back up an {opensearch} cluster by simply copying
the data directories of all of its nodes. {opensearch} may be making changes to
...
{opensearch} that can read the indices.
...
* *Snapshots*: You cannot restore snapshots from later {opensearch} versions into a cluster running an earlier {opensearch} version. For example, you cannot restore a snapshot taken in 7.6.0 to a cluster running 7.5.0.
* *Indices*: You cannot restore indices into a cluster running a version of {opensearch} that is more than _one major version_ newer than the version of {opensearch} used to snapshot the indices. For example, you cannot restore indices from a snapshot taken in 5.0 to a cluster running 7.0.
+
[NOTE]
====
The one caveat is that snapshots taken by {opensearch} 2.0 can be restored in clusters running {opensearch} 5.0.
====

Each snapshot can contain indices created in various versions of {opensearch}.
When restoring a snapshot, it
