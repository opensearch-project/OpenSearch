...
+
If `true`, the snapshot's persistent settings, index templates and ingest
pipelines are restored into the current cluster. This overwrites any existing 
cluster settings, templates and pipelines whose names match those in the snapshot.
