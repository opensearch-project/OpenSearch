...
https://search.maven.org/#search%7Cga%7C1%7Cg%3A%22org.opensearch.client%22[Maven
Central]. The minimum Java version required is `1.8`.

The low-level REST client is subject to the same release cycle as
OpenSearch. Replace the version with the desired client version, first
released with `5.0.0-alpha4`. There is no relation between the client version
and the OpenSearch version that the client can communicate with. The
low-level REST client is compatible with all OpenSearch versions.
...
--------------------------------------------------
dependencies {
    compile 'org.opensearch.client:opensearch-rest-client:{version}'
}
--------------------------------------------------
...
because it will be used to set the `Content-Type` header so that OpenSearch
...
a proxy in front of OpenSearch. There is no need to set the `Content-Type`
...
to OpenSearch and contains a comma separated list of error status codes.
...
java packages to enable logging for are `org.opensearch.client` for the
client itself and `org.opensearch.client.sniffer` for the sniffer.
