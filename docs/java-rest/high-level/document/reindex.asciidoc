...
`versionType` (as above) or setting it to internal will cause {opensearch} to blindly dump documents into the target.
Setting `versionType` to external will cause {opensearch} to preserve the version from the source, create any documents
...
+{request}+ supports reindexing from a remote {opensearch} cluster. When using a remote cluster the query should be
