...
=== Compatibility
The Java High Level REST Client requires at least Java 1.8 and depends on the OpenSearch
core project. The client version is the same as the OpenSearch version that the
...
The High Level Client is guaranteed to be able to communicate with any OpenSearch
node running on the same major version and greater or equal minor version. It
doesn't need to be in the same minor version as the OpenSearch nodes it
communicates with, as it is forward compatible meaning that it supports
communicating with later versions of OpenSearch than the one it was developed for.

The 6.0 client is able to communicate with any 6.x OpenSearch node, while the 6.1
client is for sure able to communicate with 6.1, 6.2 and any later 6.x version, but
there may be incompatibility issues when communicating with a previous OpenSearch
...
It is recommended to upgrade the High Level Client when upgrading the OpenSearch
...
https://search.maven.org/search?q=g:org.opensearch.client[Maven
Central]. The minimum Java version required is `1.8`.

The High Level REST Client is subject to the same release cycle as
OpenSearch. Replace the version with the desired client version.
...
<repositories>
    <repository>
        <id>es-snapshots</id>
        <name>opensearch snapshot repo</name>
        <url>https://snapshots.opensearch.org/maven/</url>
    </repository>
</repositories>
--------------------------------------------------
...
--------------------------------------------------
maven {
        url "https://snapshots.opensearch.org/maven/"
}
--------------------------------------------------
...
--------------------------------------------------
dependencies {
    compile 'org.opensearch.client:opensearch-rest-high-level-client:{version}'
}
--------------------------------------------------
...
<repository>
    ...
    <url>https://s3.amazonaws.com/download.opensearch.org/lucenesnapshots/83f9835</url>
...
maven {
    ...
    url 'https://s3.amazonaws.com/download.opensearch.org/lucenesnapshots/83f9835'
}
--------------------------------------------------
...
use to customize the request in ways that won't change how OpenSearch
