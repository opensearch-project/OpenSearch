...
To run the examples, index the sample seat data into OpenSearch. The examples
must be run sequentially to work correctly.

. Download the
https://download.opensearch.org/demos/painless/contexts/seats.json[seat data]. This
...
. {defguide}/running-elasticsearch.html[Start] OpenSearch. Note these
examples assume OpenSearch and Kibana are running locally. To use the Console
