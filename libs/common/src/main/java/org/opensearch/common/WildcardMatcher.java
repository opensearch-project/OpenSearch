...
/**
 * Class used to create matchers that can be used to test for matching patterns.
 *
 * WildcardMatcher supports matching using regex patterns.
 *
 * WildcardMatcher as supports wildcard (*) for matching any pattern and (?) for matching any single character.
 *
 * @opensearch.api
 */
public abstract class WildcardMatcher implements Predicate<String> {
    ...
    /**
     * Exact is a subclass of WildcardMatcher that requires the tested candidates to exactly match the pattern
     * of the Matcher
     */
    ...
    private static final class RegexMatcher extends WildcardMatcher {
        ...
        private RegexMatcher(String pattern, boolean caseSensitive) {
            if (!(pattern.length() > 1 && pattern.startsWith("/") && pattern.endsWith("/"))) {
                throw new IllegalArgumentException("Invalid regex. Must have length greater than 1 and begin and end with a /");
            }
    ...
    private static final class MatcherCombiner extends WildcardMatcher {
        ...
        MatcherCombiner(Collection<WildcardMatcher> wildcardMatchers) {
            if (!(wildcardMatchers.size() > 1)) {
                throw new IllegalArgumentException("Must provide at least 1 wildcard matcher");
            }
