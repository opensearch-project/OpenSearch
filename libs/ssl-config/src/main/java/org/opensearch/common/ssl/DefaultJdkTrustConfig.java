...
final class DefaultJdkTrustConfig implements SslTrustConfig {
    ...
    private KeyStore getSystemTrustStore() {
        ...
        if (KeyStoreUtil.inFipsMode.get() && !isBcfksTruststore(systemProperties) && !isPkcs11Truststore(systemProperties)) {
            throw new SslConfigException(
                "only %s truststores are supported in a FIPS JVM".formatted(KeyStoreUtil.FIPS_COMPLIANT_KEYSTORE_TYPES)
            );
        }
        return null;
    }
