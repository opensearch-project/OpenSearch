...
public class InternalRealm extends AuthenticatingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        if (token instanceof UsernamePasswordToken) { // This handles Username password token so need to add logic for Bearer token --
                                                      // extract subject --> get user record w/ getInternalUser
            ...
            boolean successfulAuthentication = getCredentialsMatcher().doCredentialsMatch(token, sai); // Can move JWT verification logic
                                                                                                       // into here
