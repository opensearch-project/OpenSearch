...
///
/// # Safety
/// `ptr` must be a valid pointer previously returned (as a negated i64) by `into_error_ptr`.
#[no_mangle]
...
///
/// # Safety
/// `ptr` must be a valid pointer previously returned (as a negated i64) by `into_error_ptr`, or zero.
#[no_mangle]
...
///
/// # Safety
/// `msg_ptr` must point to a valid UTF-8 byte sequence of exactly `msg_len` bytes.
#[no_mangle]
...
///
/// # Safety
/// If `ptr` is non-null and `len` is non-negative, `ptr` must point to at least `len` readable bytes.
#[no_mangle]
