...
public class PlanAlternativeSelectorTests extends OpenSearchTestCase {
    ...
    private QueryDAG forkAndSelect(
        ...
        Map<String, AnalyticsSearchBackendPlugin> registry = Map.of(dfBackend.name(), dfBackend, luceneBackend.name(), luceneBackend);
        backends.forEach(backend -> backend.bindBackends(registry));
