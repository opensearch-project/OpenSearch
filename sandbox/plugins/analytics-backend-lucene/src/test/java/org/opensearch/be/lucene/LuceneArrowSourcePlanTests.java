...
public class LuceneArrowSourcePlanTests extends OpenSearchTestCase {
    ...
    public void testAdditionalScalarColumnKinds() {
        Map<FieldType, ArrowBatchSourceFactory.ColumnKind> kinds = Map.of(
            FieldType.BOOLEAN,
            ArrowBatchSourceFactory.ColumnKind.BOOLEAN,
            FieldType.FLOAT,
            ArrowBatchSourceFactory.ColumnKind.FLOAT,
            FieldType.DOUBLE,
            ArrowBatchSourceFactory.ColumnKind.DOUBLE,
            FieldType.BINARY,
            ArrowBatchSourceFactory.ColumnKind.BINARY,
            FieldType.IP,
            ArrowBatchSourceFactory.ColumnKind.IP
        );
        for (Map.Entry<FieldType, ArrowBatchSourceFactory.ColumnKind> entry : kinds.entrySet()) {
            SqlTypeName sqlType = switch (entry.getKey()) {
                ...
                default -> throw new AssertionError(entry.getKey());
            };
            RelDataType type = nullable(sqlType);
            RelNode scan = scan(typeFactory.builder().add("value", type).build(), List.of(storage("value", entry.getKey())));
            RelNode project = LogicalProject.create(scan, List.of(), List.of(rexBuilder.makeInputRef(scan, 0)), List.of("value"));

            ArrowBatchSourceFactory.InputColumn column = arrowSourceShape(project).inputColumns().getFirst();
            assertEquals(entry.getValue(), column.kind());
            assertFalse(column.multiValued());
        }
    }
    ...
    private static final class RecordingBackend implements AnalyticsSearchBackendPlugin {
        ...
        @Override
        public byte[] compileArrowBatchSourcePlan(RelNode fragment, boolean partial) {
