 ...
 * scan itself as the initial logical fragment. The boundary node carries an {@link EngineExecutor}
...
public class BoundaryTableScanRule extends ConverterRule {
     ...
     *
     * @param engineExecutor the engine executor passed to the boundary node for bind-time execution
     * @return a new BoundaryTableScanRule
     */
    public static BoundaryTableScanRule create(EngineExecutor engineExecutor) {
    ...
    private BoundaryTableScanRule(Config config, EngineExecutor engineExecutor) {
        super(config);
