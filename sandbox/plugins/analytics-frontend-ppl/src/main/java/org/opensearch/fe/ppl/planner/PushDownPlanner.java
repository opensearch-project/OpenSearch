...
import org.opensearch.fe.ppl.planner.rules.AbsorbAggregateRule;
import org.opensearch.fe.ppl.planner.rules.AbsorbFilterRule;
import org.opensearch.fe.ppl.planner.rules.AbsorbProjectRule;
import org.opensearch.fe.ppl.planner.rules.AbsorbSortRule;
...
public class PushDownPlanner {
     ...
     * @param engineExecutor engine executor passed to boundary nodes for bind-time execution
     */
    public PushDownPlanner(EngineCapabilities capabilities, EngineExecutor engineExecutor) {
    ...
    public RelNode plan(RelNode input) {
        ...
        RelNode withBoundary = input.accept(new BoundaryTableScanShuttle(engineExecutor));
    ...
    private static class BoundaryTableScanShuttle extends RelShuttleImpl {
        ...
        BoundaryTableScanShuttle(EngineExecutor engineExecutor) {
            this.engineExecutor = engineExecutor;
        }
        ...
        public RelNode visit(TableScan scan) {
            if (scan instanceof LogicalTableScan) {
                ...
                return new OpenSearchBoundaryTableScan(scan.getCluster(), traitSet, scan.getTable(), scan, engineExecutor);
            }
            return scan;
        }
    }
