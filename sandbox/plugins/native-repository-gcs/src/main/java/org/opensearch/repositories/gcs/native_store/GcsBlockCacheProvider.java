 ...
 * node-level block cache into the GCS native object store.
 *
 ...
 * point (e.g. {@code gcs_attach_cache(store_ptr, cache_ptr, repo_name)}) which
 ...
 */
public final class GcsBlockCacheProvider implements BlockCacheProvider {

    private static final Logger logger = LogManager.getLogger(GcsBlockCacheProvider.class);

    private final GcsNativeObjectStorePlugin storePlugin;
    private final Set<String> attachedRepoNames = ConcurrentHashMap.newKeySet();

    public GcsBlockCacheProvider(final GcsNativeObjectStorePlugin storePlugin) {
        this.storePlugin = Objects.requireNonNull(storePlugin, "storePlugin must not be null");
    }

    @Override
    public String repositoryType() {
        return GcsNativeObjectStorePlugin.TYPE;
    }
    ...
    public void attach(final BlockCacheHandle handle, final RepositoryMetadata metadata) {
        ...
        if (attachedRepoNames.add(metadata.name()) == false) {
            logger.debug("GcsBlockCacheProvider.attach skipped (already attached) for repo [{}]", metadata.name());
            return;
        }
        if (handle.getCache() instanceof FoyerBlockCache foyer) {
            ...
            // GcsNativeBridge.attachCache(storePlugin.nativeStorePtrFor(metadata), nativeCachePtr, metadata.name());
            logger.info(
                "GcsBlockCacheProvider attach (structural; native FFM pending): cachePtr={}, repo=[{}], type=[{}]",
                nativeCachePtr,
                metadata.name(),
                metadata.type()
            );
            ...
            logger.info(
                "GcsBlockCacheProvider: handle is {} (not FoyerBlockCache); skipping native attach for repo [{}]",
