...
/**
 * Asserts standard numeric metric aggregations over a Parquet-resident {@code value} field read
 * through {@code ParquetNumericDocValues}: the field is absent from the lucene secondary (which holds
 * only the keyword {@code name} and the row-id), so no Lucene-native doc values can serve it.
 */
