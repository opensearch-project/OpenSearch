 ...
 * node-level block cache into the FS native object store.
 *
 ...
 * point (e.g. {@code fs_attach_cache(store_ptr, cache_ptr, repo_name)}) which
 ...
 */
public final class FsBlockCacheProvider implements BlockCacheProvider {

    private static final Logger logger = LogManager.getLogger(FsBlockCacheProvider.class);

    private final FsNativeObjectStorePlugin storePlugin;
    private final Set<String> attachedRepoNames = ConcurrentHashMap.newKeySet();

    public FsBlockCacheProvider(final FsNativeObjectStorePlugin storePlugin) {
        this.storePlugin = Objects.requireNonNull(storePlugin, "storePlugin must not be null");
    }

    @Override
    public String repositoryType() {
        return FsNativeObjectStorePlugin.TYPE;
    }
    ...
    public void attach(final BlockCacheHandle handle, final RepositoryMetadata metadata) {
        ...
        if (attachedRepoNames.add(metadata.name()) == false) {
            logger.debug("FsBlockCacheProvider.attach skipped (already attached) for repo [{}]", metadata.name());
            return;
        }
        if (handle.getCache() instanceof FoyerBlockCache foyer) {
            ...
            // FsNativeBridge.attachCache(storePlugin.nativeStorePtrFor(metadata), nativeCachePtr, metadata.name());
            logger.info(
                "FsBlockCacheProvider attach (structural; native FFM pending): cachePtr={}, repo=[{}], type=[{}]",
                nativeCachePtr,
                metadata.name(),
                metadata.type()
            );
            ...
            logger.info(
                "FsBlockCacheProvider: handle is {} (not FoyerBlockCache); skipping native attach for repo [{}]",
