...
struct QueryStreamHandle {
    stream: datafusion::physical_plan::stream::RecordBatchStreamAdapter<cross_rt_stream::CrossRtStream>,
...
pub extern "system" fn Java_org_opensearch_be_datafusion_jni_NativeBridge_executeQueryAsync(
    ...
    // Create per-query memory tracking context
    let runtime = unsafe { &*(runtime_ptr as *const api::DataFusionRuntime) };
    ...
    let result = tokio_rt_mgr.io_runtime.block_on(unsafe {
        api::execute_query(shard_view_ptr as i64, &table_name, &plan_bytes, runtime_ptr as i64, tokio_rt_mgr, query_pool)
    });

    with_jni_env(|env| match result {
        Ok(stream_ptr) => {
            // Wrap stream with query context for automatic cleanup on close
            let stream = unsafe { *Box::from_raw(stream_ptr as *mut datafusion::physical_plan::stream::RecordBatchStreamAdapter<cross_rt_stream::CrossRtStream>) };
...
pub extern "system" fn Java_org_opensearch_be_datafusion_jni_NativeBridge_streamGetSchema(
    ...
    let handle = unsafe { &*(stream_ptr as *const QueryStreamHandle) };
    let schema = handle.stream.schema();
    match FFI_ArrowSchema::try_from(schema.as_ref()) {
        Ok(ffi_schema) => {
            let ptr = Box::into_raw(Box::new(ffi_schema)) as jlong;
            set_action_listener_ok(env, listener, ptr);
        }
        Err(e) => set_action_listener_error(env, listener, &DataFusionError::Execution(format!("Schema conversion failed: {}", e))),
    }
}
...
pub extern "system" fn Java_org_opensearch_be_datafusion_jni_NativeBridge_streamNext(
    ...
    let result = manager.io_runtime.block_on(async {
        let handle = unsafe { &mut *(stream_ptr as *mut QueryStreamHandle) };
        let batch = handle.stream.try_next().await?;
        match batch {
            Some(batch) => {
...
pub extern "system" fn Java_org_opensearch_be_datafusion_jni_NativeBridge_streamClose(
    ...
    if stream_ptr != 0 {
        let _ = unsafe { Box::from_raw(stream_ptr as *mut QueryStreamHandle) };
    }
}
