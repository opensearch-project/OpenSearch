...
pub extern "system" fn Java_org_opensearch_be_datafusion_jni_NativeBridge_createDatafusionReader(
    ...
    let mut filenames = match parse_string_arr(&mut env, files) {
        Ok(f) => f,
        Err(e) => {
            let _ = env.throw_new(
                "java/lang/IllegalArgumentException",
    ...
    // Resolve the object store for this URL from DataFusion's default registry
    let default_rt = datafusion::execution::runtime_env::RuntimeEnvBuilder::new()
        .build()
        .unwrap();

    // TODO : Store should be plugged in based on hot or warm node
    ...
    // Use IO runtime to fetch object metadata via the resolved store
...
pub extern "system" fn Java_org_opensearch_be_datafusion_jni_NativeBridge_streamGetSchema(
    ...
    match FFI_ArrowSchema::try_from(schema.as_ref()) {
        Ok(ffi_schema) => {
            ...
            set_action_listener_ok(&mut env, listener, schema_ptr as jlong);
        }
        Err(e) => {
            set_action_listener_error(
                &mut env,
...
pub extern "system" fn Java_org_opensearch_be_datafusion_jni_NativeBridge_streamNext(
    ...
    // TODO: convert to io_runtime.spawn for async handoff
    io_runtime.block_on(async move {
        let stream =
            unsafe { &mut *(stream_ptr as *mut RecordBatchStreamAdapter<CrossRtStream>) };
        ...
        with_jni_env(|env| {
            set_action_listener_error_global(
                ...
                &DataFusionError::NotImplemented(format!(
                    "Tree query execution stub: tree parsed OK, contextId={}, table={}, partitions={}, plan_bytes={}",
                    context_id, table_name_str, num_parts, plan_bytes.len()
                )),
            );
        });
    });
