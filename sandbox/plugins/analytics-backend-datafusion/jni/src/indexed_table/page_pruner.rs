...
impl PagePruner {
    ...
    /// Compute page ranges for a single filter (column, op, value) on a row group.
    ...
    /// Return all row IDs within pages that match a specific filter for a row group.
    ///
    /// Used by tree_eval for PredicateLeaf evaluation in Phase 1 (prefetch).
    pub fn candidate_row_ids_for_filter(
        ...
        let ranges = match self.compute_page_ranges_for_filter(col_name, op, value, rg_idx, rg_first_row) {
            Some(r) if r.is_empty() => return vec![],
            Some(r) => r,
            None => return (rg_first_row..rg_first_row + rg_num_rows).collect(),
        };
