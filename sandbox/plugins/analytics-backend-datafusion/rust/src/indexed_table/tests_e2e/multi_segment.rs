        ...
        Arc::new(move |segment, _chunk| {
            ...
            let eval: Arc<dyn RowGroupBitsetSource> = Arc::new(
                crate::indexed_table::eval::single_collector::SingleCollectorEvaluator::new(
                    collector,
                    pruner,
                    crate::indexed_table::index::BitsetMode::And,
                ),
            );
            Ok(eval)
        })
...
use super::super::bool_tree::ResolvedPredicate;
...
async fn wide_multi_segment_collector_and_two_predicates() {
    ...
    // Expected per row i: true && (i*10 > 50) && (i%7 < 3)
    //                   = (i > 5) && (i%7 in {0,1,2})
    ...
    let preds = vec![
        ResolvedPredicate {
            column: "price".into(),
            op: Operator::Gt,
            value: ScalarValue::Int32(Some(50)),
        },
        ResolvedPredicate {
            column: "qty".into(),
            op: Operator::Lt,
    ...
    for np in [1usize, 3, 5] {
        let rows = run_wide_segments(clone_wide_specs(&specs), tree.clone(), preds.clone(), np).await;
        assert_eq!(rows, expected, "np={} failed", np);
    }
}
...
async fn wide_multi_segment_or_of_predicates_under_collector() {
    ...
    // Expected per row i: (i%3 == 0) || (i%2 == 0)
    let specs = wide_four_seg_specs();
    let tree = BoolNode::And(vec![
        BoolNode::Collector { query_bytes: Arc::from(&[0u8][..]) },
        BoolNode::Or(vec![
    ...
    let preds = vec![
        ResolvedPredicate {
            column: "region".into(),
            op: Operator::Eq,
            value: ScalarValue::Utf8(Some("us-east".into())),
        },
        ResolvedPredicate {
            column: "active".into(),
            op: Operator::Eq,
...
async fn wide_multi_segment_not_and_three_column_predicates() {
    ...
    // Expected per row i: !(i*10 < 30) && (i%7 > 2) && (region(i) != "eu-west")
    //                   = (i >= 3) && (i%7 > 2) && (i%3 != 2)
    let specs = wide_four_seg_specs();
    let tree = BoolNode::And(vec![
        BoolNode::Collector { query_bytes: Arc::from(&[0u8][..]) },
    ...
    let preds = vec![
        ...
        ResolvedPredicate {
            column: "region".into(),
            op: Operator::NotEq,
            value: ScalarValue::Utf8(Some("eu-west".into())),
        },
    ];
...
async fn wide_multi_segment_deep_tree_four_predicate_columns() {
    ...
    // Exercises: nested AND inside OR inside top-level AND, 4 distinct predicate
    // columns, across 4 segments with 4 RGs each (16 RGs total).
    let specs = wide_four_seg_specs();
    let tree = BoolNode::And(vec![
        BoolNode::Collector { query_bytes: Arc::from(&[0u8][..]) },
        BoolNode::Or(vec![
            ...
            BoolNode::And(vec![
                BoolNode::Predicate { predicate_id: 2 },
                BoolNode::Predicate { predicate_id: 3 },
            ]),
        ]),
    ]);
    let preds = vec![
        ResolvedPredicate {
            column: "price".into(),
            op: Operator::GtEq,
            value: ScalarValue::Int32(Some(100)),
        },
        ResolvedPredicate {
            column: "qty".into(),
            op: Operator::Eq,
        ...
        ResolvedPredicate {
            column: "region".into(),
            op: Operator::Eq,
            value: ScalarValue::Utf8(Some("us-west".into())),
        },
        ResolvedPredicate {
            column: "active".into(),
            op: Operator::Eq,
