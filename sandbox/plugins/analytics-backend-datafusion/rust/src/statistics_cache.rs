...
impl CustomStatisticsCache {
    ...
    pub fn update_size_limit(&self, new_limit: usize) -> CacheResult<()> {
        ...
        if current_size > new_limit {
            ...
            // Perform eviction using remove_internal (works with &self)
            let candidates = {
...
impl CacheAccessor<Path, Arc<Statistics>> for CustomStatisticsCache {
    ...
    fn put_with_extra(
        ...
        if current_size + memory_size > (self.size_limit.load(Ordering::Relaxed) as f64 * self.eviction_threshold) as usize {
            let target_eviction = (current_size + memory_size) - (self.size_limit.load(Ordering::Relaxed) as f64 * 0.6) as usize;
            let candidates = {
                if let Ok(policy_guard) = self.policy.lock() {
                    policy_guard.select_for_eviction(target_eviction)
                } else { vec![] }
