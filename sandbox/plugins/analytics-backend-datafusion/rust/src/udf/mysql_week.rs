...
//! `mysql_week(date [, mode])` — MySQL `WEEK()` semantics.
//!
//! Cluster F cases 1+2: PPL's `WEEK(date)` and `WEEK_OF_YEAR(date)` follow
//! MySQL's modes, not the ISO week. Mode 0 (default) is Sunday-first; week 1
//! starts the first Sunday of the year, dates before that are week 0. Modes 1,
//! 2, 3 cover Monday-first / Sunday-first-with-fold / Monday-first-with-fold
//! variants.
//!
//! DataFusion's stock `date_part('week', ts)` returns the ISO week number, which
//! disagrees with MySQL's mode 0 default — e.g. 2008-02-20 (Wednesday) is ISO
//! week 8 but MySQL mode 0 week 7. This UDF takes the operand directly and
//! computes per-mode.
...
pub fn register_all(ctx: &SessionContext) {
    ctx.register_udf(ScalarUDF::from(MysqlWeekUdf::new()));
}

#[derive(Debug)]
pub struct MysqlWeekUdf {
    signature: Signature,
}
...
udf_identity!(MysqlWeekUdf, "mysql_week");

impl ScalarUDFImpl for MysqlWeekUdf {
    ...
    fn return_type(&self, arg_types: &[DataType]) -> Result<DataType> {
        if arg_types.is_empty() || arg_types.len() > 2 {
            return plan_err!("mysql_week expects 1 or 2 arguments, got {}", arg_types.len());
        }
        Ok(DataType::Int32)
    }
    fn coerce_types(&self, arg_types: &[DataType]) -> Result<Vec<DataType>> {
        ...
        match &arg_types[0] {
            DataType::Date32 | DataType::Date64 | DataType::Timestamp(_, _) | DataType::Utf8 | DataType::LargeUtf8 | DataType::Utf8View => {
                out.push(DataType::Date32);
            }
            other => return plan_err!("mysql_week: arg 0 expected date/timestamp/string, got {other:?}"),
        }
        if arg_types.len() == 2 {
            match &arg_types[1] {
                DataType::Int8 | DataType::Int16 | DataType::Int32 | DataType::Int64 | DataType::UInt8 | DataType::UInt16 | DataType::UInt32 | DataType::UInt64 => {
                    out.push(DataType::Int32);
                }
                other => return plan_err!("mysql_week: arg 1 expected integer mode, got {other:?}"),
            }
        }
        Ok(out)
    }
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        let dates = date_arg
            ...
            .ok_or_else(|| datafusion::error::DataFusionError::Internal(
                "mysql_week: arg 0 not coerced to Date32".to_string(),
            ))?;
        ...
        for i in 0..n {
            ...
            let date = NaiveDate::from_num_days_from_ce_opt(days + 719_163)
                .ok_or_else(|| datafusion::error::DataFusionError::Execution(
                    format!("mysql_week: invalid Date32 day count {days}"),
                ))?;
            builder.append_value(mysql_week_number(date, mode) as i32);
        }
        Ok(ColumnarValue::Array(Arc::new(builder.finish()) as ArrayRef))
    }
}

/// Compute the MySQL week number for a date under the given mode (0..7).
/// MySQL defines 8 modes; this table follows
/// https://dev.mysql.com/doc/refman/8.0/en/date-and-time-functions.html#function_week.
/// Modes 0/2/5/7 use simple "first {first-day} of the year" counting; modes
/// 1/3/4/6 use the ≥4-days-in-year rule for week 1. Range 0-53 vs 1-53 only
/// affects how week 0 is reported.
fn mysql_week_number(date: NaiveDate, mode: i32) -> u32 {
...
mod tests {
    ...
    #[test]
    fn mysql_week_default_mode_zero() {
        // Cluster F case 1: 2008-02-20 (Wed) → MySQL mode 0 = 7.
        let d = NaiveDate::from_ymd_opt(2008, 2, 20).unwrap();
        assert_eq!(mysql_week_number(d, 0), 7);
    }

    #[test]
    fn mysql_week_table_from_test_source() {
        ...
        for (d, mode, want) in cases {
            assert_eq!(mysql_week_number(*d, *mode), *want, "date={d}, mode={mode}");
        }
    }
}
