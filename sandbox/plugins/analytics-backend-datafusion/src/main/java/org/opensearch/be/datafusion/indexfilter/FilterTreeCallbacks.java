...
public final class FilterTreeCallbacks {
    ...
    private enum GiveUpCause {
        /**
         * No binding for this {@code contextId}. Under the refcounted lifecycle this is a genuine
         * bug — bindings outlive every native handle — so either {@link #unregister(long)} was
         * used where {@link #requestClose(long)} belonged, or {@link #register} never ran.
         */
        NO_BINDING,
        ...
        if (cause == GiveUpCause.NO_BINDING) {
            LOGGER.warn(
                "{}(contextId={}, key={}){} returning -1: NO BINDING registered — the query's binding was "
                    + "removed while a native collector call was still in flight, or register() never ran. "
    ...
    public static void unregister(long contextId) {
        ...
        // want to see are the ones that arrive AFTER teardown (a native collector call still in
        // flight). Dropping the dedup marker here would let those late calls re-log once per row
        // group — exactly the flood the dedup exists to prevent. The FIFO cap in firstGiveUp
        // bounds the set instead.
    }
