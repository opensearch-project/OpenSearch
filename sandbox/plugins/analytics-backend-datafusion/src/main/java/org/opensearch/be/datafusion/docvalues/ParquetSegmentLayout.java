 ...
 * The stamping side ships with the composite-engine integration (#22752). There is no directory scan,
 * because a shard's Parquet directory holds one file per writer generation and only the stamped path
 * identifies the one whose rows are this segment's documents; guessing would risk reading the wrong file.
 *
