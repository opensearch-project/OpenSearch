...
public final class NativeBridge {
    ...
    private static synchronized void loadNativeLibrary() {
        ...
        // Extract from jar resources to a temp file
        String libName = System.mapLibraryName("opensearch_datafusion_jni");
        String resourcePath = "/native/" + libName;
        try (java.io.InputStream is = NativeBridge.class.getResourceAsStream(resourcePath)) {
            if (is == null) {
                throw new ExceptionInInitializerError("Native library not found in resources: " + resourcePath);
            }
            java.nio.file.Path tempFile = java.nio.file.Files.createTempFile("opensearch_datafusion_jni", libName.substring(libName.lastIndexOf('.')));
            tempFile.toFile().deleteOnExit();
            java.nio.file.Files.copy(is, tempFile, java.nio.file.StandardCopyOption.REPLACE_EXISTING);
            System.load(tempFile.toAbsolutePath().toString());
            loaded = true;
        } catch (java.io.IOException e) {
            throw new ExceptionInInitializerError("Failed to extract native library: " + e.getMessage());
        }
    }
     ...
     * @param contextId the query context ID for per-query memory tracking
