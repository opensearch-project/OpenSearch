...
public class AggregationUDFIT extends OpenSearchIntegTestCase {
    ...
    private void createBankIndex() throws Exception {
        ...
        CreateIndexResponse response = client().admin().indices().prepareCreate(BANK_INDEX).setSettings(indexSettings).setMapping(mapping).get();
    ...
    private void indexBankDocs() {
        ...
        client().prepareIndex(BANK_INDEX).setId("1")
            .setSource("account_number", 1, "firstname", "Amber JOHnny", "lastname", "Duke Willmington", "balance", 39225L, "age", 32, "gender", "M")
            .get();
