...
public class TimestampFunctionAdapterTests extends OpenSearchTestCase {
     ...
     * temporal columns (precision_timestamp / date / time). Rewrites to
     * {@code from_unixtime(to_unixtime(date) + to_unixtime(time))} so it lowers
     * through existing signatures.
     */
    public void testAdaptShapeETwoArgTemporalColumnsRewriteToFromUnixtime() {
