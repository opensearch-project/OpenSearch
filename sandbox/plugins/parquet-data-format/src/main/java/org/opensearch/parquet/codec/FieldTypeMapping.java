...
public final class FieldTypeMapping {
    ...
    // TODO(type-coverage): the following OpenSearch types can be WRITTEN to Parquet by the composite
    // engine but are intentionally NOT mapped here, so the codec serves no doc values for them:
    // half_float, unsigned_long, scaled_float, token_count, version, wildcard, constant_keyword,
    // flat_object, match_only_text. Two of these are trivially correct to add (match_only_text ->
    // BINARY like text; token_count -> NUMERIC/INT32) and should be added with tests. scaled_float is
    // a correctness landmine: it is written as a RAW SCALED long, so mapping it to NUMERIC without
    // applying the scaling factor on read would return silently wrong values — implement read-side
    // scaling or reject it at index creation. half_float/unsigned_long need real decode semantics.
    // A follow-up change should move rejection of unmapped types to index-creation time
    // (ParquetIndexCreationValidator) so an unmapped field fails loudly rather than silently serving
    // no doc values. Until then, forType/validate below throw for any unmapped type (loud at query
    // time), and the leaf reader skips unmapped fields entirely.
