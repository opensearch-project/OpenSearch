...
///
/// IPC counterpart of `merge_sorted`. Accepts pre-known Arrow schema and
/// per-file row counts (from `SortingChunkedWriter`) instead of reading
/// Parquet metadata. Same merge algorithm and MergeContext infrastructure.
pub fn merge_sorted_from_ipc(
    ...
    if input_files.len() != file_row_counts.len() {
        return Err(super::MergeError::Logic(format!(
            ...
            input_files.len(), file_row_counts.len()
        )));
    }
    ...
    log_info!(
        "[RUST] Starting streaming IPC merge: {} input files, sort_columns={:?}, \
         batch_size={}, flush_rows={}, merge_threads={}, output='{}'",
        input_files.len(), sort_columns, batch_size, output_flush_rows,
        pool.current_num_threads(), output_path
    );
    ...
    for (file_id, path) in input_files.iter().enumerate() {
        let (cursor, projected_schema, parquet_descr, generation, row_count) =
            FileCursor::new_from_ipc(
                path, file_id, sort_columns, nulls_first, batch_size,
                schema.clone(), output_writer_generation, file_row_counts[file_id],
            )?;
        cursors.push(cursor);
        arrow_schemas.push(projected_schema.as_ref().clone());
        parquet_descriptors.push(parquet_descr);
    ...
    while let Some(item) = heap.pop() {
        ...
        loop {
            ...
            while lo + 1 < hi {
                let mid = lo + (hi - lo) / 2;
                let mid_val = get_sort_values(
                    batch, mid, &cursor.sort_col_indices,
                    &cursor.sort_col_types, &cursor.nulls_first,
                )?;
                if cmp_sort_values(&mid_val, heap_top, reverse_sorts) != Ordering::Greater {
                    lo = mid;
                } else {
    ...
    log_info!(
        "[RUST] IPC Merge complete: {} total rows, {} row groups, crc32={:#010x}",
        stats.metadata.file_metadata().num_rows(),
        stats.metadata.num_row_groups(),
        stats.crc32
    );
