...
public class FiltersBucketTranslator implements BucketTranslator<FiltersAggregationBuilder> {
    ...
    @Override
    public BucketOrder getBucketOrder(FiltersAggregationBuilder agg) {
        return null;
    }
    ...
    @Override
    public InternalAggregation toBucketAggregation(FiltersAggregationBuilder agg, Iterable<BucketEntry> buckets) {
    ...
    public RexNode buildOtherBucketCondition(List<RexNode> filterConditions, ConversionContext ctx) throws ConversionException {
        ...
        if (filterConditions.size() == 1) {
            disjunction = filterConditions.getFirst();
        } else {
            disjunction = ctx.getRexBuilder().makeCall(SqlStdOperatorTable.OR, filterConditions);
        }
     ...
     */
    public List<FilterPlanDescriptor> resolveFilterPlans(FiltersAggregationBuilder agg, RexNode parentCondition, ConversionContext ctx)
