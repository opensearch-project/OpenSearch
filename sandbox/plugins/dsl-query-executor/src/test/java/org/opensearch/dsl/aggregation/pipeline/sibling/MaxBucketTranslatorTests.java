...
public class MaxBucketTranslatorTests extends OpenSearchTestCase {

    private final MaxBucketTranslator translator = new MaxBucketTranslator();
    private final ConversionContext ctx = TestUtils.createContext();

    public void testGetBuilderClass() {
        assertEquals(MaxBucketPipelineAggregationBuilder.class, translator.getBuilderClass());
    }

    public void testTranslateProducesMaxAggregate() throws ConversionException {
        RelNode input = buildBaseAggregate();
        var builder = new MaxBucketPipelineAggregationBuilder("max_rev", "total_revenue");
        ...
        assertEquals(SqlKind.MAX, agg.getAggCallList().get(0).getAggregation().getKind());
        assertEquals("max_rev", agg.getAggCallList().get(0).getName());
    }

    public void testTranslateThrowsForUnknownPath() {
        RelNode input = buildBaseAggregate();
        var builder = new MaxBucketPipelineAggregationBuilder("bad", "nonexistent");
    ...
    public void testToInternalAggregationWithValidRow() {
        var builder = new MaxBucketPipelineAggregationBuilder("max_rev", "by_brand>revenue");
        var result = (InternalSimpleValue) translator.toInternalAggregation(builder, new Object[]{380.0});

        assertEquals(380.0, result.value(), 0.0001);
    }
    ...
    public void testToInternalAggregationWithNullRow() {
        ...
        assertEquals(Double.NEGATIVE_INFINITY, result.value(), 0.0001);
    }
