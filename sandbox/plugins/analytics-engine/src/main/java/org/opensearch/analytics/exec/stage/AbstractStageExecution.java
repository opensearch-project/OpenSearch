...
public abstract class AbstractStageExecution implements StageExecution {
    ...
    /**
     * Template: materialise task list (possibly async — see {@link #materializeTasksAsync}),
     * publish on completion. Final — subclasses customise via {@link #materializeTasks()} for
     * synchronous materialisation or {@link #materializeTasksAsync} for work that needs to
     * defer publication (e.g. a network round-trip).
     *
     * <p>{@code start()} may return BEFORE the stage has transitioned to RUNNING. Callers
     * that need to act on the transition (e.g. dispatching tasks) must register a
     * {@link #addStateListener state listener} rather than polling {@link #getState()} right
     * after this call.
     */
    @Override
    ...
    /**
     * Async variant of {@link #materializeTasks()}. Default impl runs the sync method and
     * invokes the listener inline. Override when the work needs to defer publication —
     * e.g. {@code ShardFragmentStageExecution} dispatches a can-match round-trip before
     * publishing.
     *
     * <p>Implementations MUST eventually call exactly one of {@code listener.onResponse}
     * (which transitions the stage via {@link #publishTasksAndStart}) or
     * {@code listener.onFailure} (which fails the stage). Synchronous exceptions thrown
     * before either are caught by {@link #start()} and treated as failures.
     */
