 ...
 * the underlying {@link ShardId}; on dispatch failure, {@link #nextCopy(Exception)} advances
 * to the next copy on a different node, mirroring the search API's
...
public final class ShardExecutionTarget extends ExecutionTarget {
    ...
    /**
     * Legacy constructor for callers that don't track replica copies (tests, fragments where
     * routing isn't available). {@link #nextCopy(Exception)} on a target built this way returns null,
     * preserving the original "no failover" behavior.
     */
    public ShardExecutionTarget(DiscoveryNode node, ShardId shardId) {
        this(node, shardId, null, null);
    }
    ...
    public ShardExecutionTarget nextCopy(@Nullable Exception cause) {
        ...
        return new ShardExecutionTarget(nextNode, next.shardId(), remainingCopies, clusterState);
    }
}
