...
/**
 * Common-subexpression sharing in {@link DAGBuilder} ({@code analytics.planner.subplan_reuse.enabled}).
 *
...
public class SharedSubplanReuseTests extends BasePlannerRulesTests {
    ...
    public void testReuseDisabled_duplicateAggregateIsComputedTwice() {
        ...
        assertEquals("both copies of the aggregate are cut as their own stages with sub-plan reuse off", 2, stageInputScans(dag).size());
     ...
     * that and rebuild without sub-plan reuse — a missed sub-plan reuse is slower, a double read of a once-consumable input is wrong.
     */
