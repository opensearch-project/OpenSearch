 ...
 *
 * <p><b>The golden is the TRUE answer, not parquet's answer.</b> Goldens are generated out-of-band by
 * running the dataset on a vanilla OpenSearch index (default Lucene backend, no parquet settings) — the
 * reference implementation — and committed. This suite only validates the parquet response against that
 * true answer, so any place parquet deviates (null text {@code _source}, multi-valued keyword /
 * custom-id / geo / nested failures) correctly shows red. This mirrors the PPL/SQL suite in
 * {@code analytics-engine-rest}, whose expected files are known-correct answers, never the engine's own
 * output.
 *
 * <p><b>Goldens are the TRUE answer, not parquet's answer.</b> They are generated out-of-band by running
 * each dataset on a vanilla OpenSearch index (default Lucene backend, no parquet settings) — the reference
 * implementation — and committing that response. This suite only ever <i>validates</i> the parquet
 * response against the committed golden, so any place parquet deviates (null text {@code _source},
 * multi-valued keyword / custom-id / geo / nested failures) correctly shows red. This mirrors the PPL/SQL
 * suite in {@code analytics-engine-rest}, whose expected files are known-correct answers, never the
 * engine's own output.
 */
