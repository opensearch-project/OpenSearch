 ...
 */
public class FunctionsPplIT extends AnalyticsRestTestCase {
    ...
    private void ensureDataProvisioned() throws Exception {
        if (dataProvisioned == false) {
            DatasetProvisioner.provision(client(), FunctionsTestHelper.DATASET);
    ...
    public void testFunctionsPplQueries() throws Exception {
        ...
        List<Integer> queryNumbers = DatasetQueryRunner.discoverQueryNumbers(FunctionsTestHelper.DATASET, "ppl")
            .stream()
            .filter(n -> SKIP_QUERIES.contains(n) == false)
            .toList();
        assertFalse("No PPL queries discovered", queryNumbers.isEmpty());
        logger.info("Running {} PPL function testing PPL queries: {}", queryNumbers.size(), queryNumbers);

        List<String> failures = DatasetQueryRunner.runQueries(
            client(),
            FunctionsTestHelper.DATASET,
            "ppl",
            "ppl",
            queryNumbers,
        ...
        if (failures.isEmpty() == false) {
            fail("PPL function testing PPL query failures (" + failures.size() + " of " + queryNumbers.size() + "):\n" + String.join("\n", failures));
        }
    }
}
