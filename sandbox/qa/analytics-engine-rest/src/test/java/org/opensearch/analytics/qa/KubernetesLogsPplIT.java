 ...
 */
public class KubernetesLogsPplIT extends AnalyticsRestTestCase {
    ...
    public void testKubernetesLogsPplQueries() throws Exception {
        ...
        List<Integer> queryNumbers = DatasetQueryRunner.discoverQueryNumbers(KubernetesLogsTestHelper.DATASET, "ppl")
            .stream()
            .filter(n -> SKIP_QUERIES.contains(n) == false)
            .toList();
        assertFalse("No PPL queries discovered", queryNumbers.isEmpty());
        logger.info("Running {} Kubernetes log analysis PPL queries: {}", queryNumbers.size(), queryNumbers);

        List<String> failures = DatasetQueryRunner.runQueries(
            client(),
            KubernetesLogsTestHelper.DATASET,
            "ppl",
            "ppl",
            queryNumbers,
        ...
        if (failures.isEmpty() == false) {
            fail("Kubernetes log analysis PPL query failures (" + failures.size() + " of " + queryNumbers.size() + "):\n" + String.join("\n", failures));
        }
    }
}
