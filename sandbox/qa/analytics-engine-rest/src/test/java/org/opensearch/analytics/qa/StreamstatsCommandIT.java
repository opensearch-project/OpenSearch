...
public class StreamstatsCommandIT extends AnalyticsRestTestCase {
    ...
    // ── distinct_count / dc — blocked at PPL parser stage ─────────────────────
    // BackendPlanAdapter does rewrite RexOver(DISTINCT_COUNT_APPROX) → APPROX_COUNT_DISTINCT
    // (covered by WindowFunctionAdaptersTests), but PPL parser fails earlier: sql plugin
    // registers the DISTINCT_COUNT_APPROX UDAF on PPLFuncImpTable only inside
    // OpenSearchExecutionEngine's constructor (V2/V3 path), which is never triggered on
    // the unified-query / analytics-engine path. Until that registration is moved upstream,
    // dc()/distinct_count() requests fail at parse time with "Cannot resolve function".
