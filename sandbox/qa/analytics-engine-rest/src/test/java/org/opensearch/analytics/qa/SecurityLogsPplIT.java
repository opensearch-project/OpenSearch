 ...
 */
public class SecurityLogsPplIT extends AnalyticsRestTestCase {
    ...
    public void testSecurityLogsPplQueries() throws Exception {
        ...
        List<Integer> queryNumbers = DatasetQueryRunner.discoverQueryNumbers(SecurityLogsTestHelper.DATASET, "ppl")
            .stream()
            .filter(n -> SKIP_QUERIES.contains(n) == false)
            .toList();
        assertFalse("No PPL queries discovered", queryNumbers.isEmpty());
        logger.info("Running {} Security Logs PPL queries: {}", queryNumbers.size(), queryNumbers);

        List<String> failures = DatasetQueryRunner.runQueries(
            client(),
            SecurityLogsTestHelper.DATASET,
            "ppl",
            "ppl",
            queryNumbers,
        ...
        if (failures.isEmpty() == false) {
            fail("Security Logs PPL query failures (" + failures.size() + " of " + queryNumbers.size() + "):\n" + String.join("\n", failures));
        }
    }
}
