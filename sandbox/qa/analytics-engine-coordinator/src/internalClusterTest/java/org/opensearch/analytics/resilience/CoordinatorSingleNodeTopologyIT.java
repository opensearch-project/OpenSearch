 ...
 * any remote dispatch. Lives in its own IT class because the Tokio runtime manager
 * is JVM-global; tests that stop nodes mid-run shut it down for the whole test JVM,
 * which is why this can't share a class with {@code CoordinatorResilienceIT}.
 */
...
public class CoordinatorSingleNodeTopologyIT extends OpenSearchIntegTestCase {

    private static final String INDEX = "topology_single_node_idx";
    ...
    /**
     * Single node, 2 shards, both primaries on the same node. SUM aggregate must
     * still flow through the coordinator-reduce path even though both fragments
     * execute locally — verifying same-node fan-in.
     */
    public void testSingleNodeMultiShardSum() throws Exception {
        ...
        try {
            ...
            throw new AssertionError("single-node multi-shard query failed", e);
        }
    }
}
