 ...
 * </ul>
 *
 * Each blocklist pattern is a suffix match on the path. Empty patterns are not allowed.
 */
final class BlocklistedPathPatternMatcher {
    private final Pattern pattern;

    /**
     * Constructs a new <code>BlocklistedPathPatternMatcher</code> instance from the provided suffix pattern.
     *
     * @param p The suffix pattern. Must be a non-empty string.
     */
    BlocklistedPathPatternMatcher(String p) {
        // guard against accidentally matching everything as an empty string lead to the pattern ".*" which matches everything
        if (p == null || p.trim().isEmpty()) {
            throw new IllegalArgumentException("Empty blocklist patterns are not supported");
        }
