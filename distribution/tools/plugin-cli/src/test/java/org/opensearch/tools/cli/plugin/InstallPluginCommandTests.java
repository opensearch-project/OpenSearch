...
public class InstallPluginCommandTests extends OpenSearchTestCase {
    ...
    public void testInstallPluginWithSatisfiedDependency() throws Exception {
        Tuple<Path, Environment> env = createEnv(fs, temp);

        // install the dependency first
        Path depDir = createPluginDir(temp);
        String depZip = createPluginUrl("dep-plugin", depDir);
        installPlugin(depZip, env.v1());
    ...
    public void testInstallPluginAutoInstallsMissingDependency() throws Exception {
        Tuple<Path, Environment> env = createEnv(fs, temp);

        // build the dep zip but do NOT install it yet
        Path depDir = createPluginDir(temp);
        String depZip = createPluginUrl("dep-plugin", depDir);
        ...
        assertPlugin("dep-plugin", depDir, env.v2());
        assertPlugin("fake", pluginDir, env.v2());
    }

    public void testInstallPluginAutoInstallsDependencyFromSiblingZip() throws Exception {
        // real filesystem only — sibling lookup uses Path.of(URI) which needs a real FS
        assumeTrue("real filesystem", isReal);
        Tuple<Path, Environment> env = createEnv(fs, temp);

        // place both zips in the same directory, simulating a build/distributions folder
        Path distDir = temp.apply("distributions");
        Path depDir = createPluginDir(temp);
        writePlugin("dep-plugin", depDir);
        Path depZip = distDir.resolve("dep-plugin-1.0.zip");
        ...
        // install from the sibling directory — dep-plugin should be found automatically
        installPlugin(pluginZip.toUri().toURL().toString(), env.v1());

        assertPlugin("dep-plugin", depDir, env.v2());
        assertPlugin("fake", pluginDir, env.v2());
    }

    public void testInstallPluginWithMultipleDependenciesAllPresent() throws Exception {
        Tuple<Path, Environment> env = createEnv(fs, temp);

        Path dep1Dir = createPluginDir(temp);
        String dep1Zip = createPluginUrl("dep-one", dep1Dir);
        installPlugin(dep1Zip, env.v1());
        Path dep2Dir = createPluginDir(temp);
    ...
    public void testInstallPluginAutoInstallsAllMissingDependencies() throws Exception {
        Tuple<Path, Environment> env = createEnv(fs, temp);

        // neither dep is installed yet
        Path dep1Dir = createPluginDir(temp);
        String dep1Zip = createPluginUrl("dep-one", dep1Dir);
        Path dep2Dir = createPluginDir(temp);
        String dep2Zip = createPluginUrl("dep-two", dep2Dir);
        ...
        assertPlugin("dep-two", dep2Dir, env.v2());
        assertPlugin("fake", pluginDir, env.v2());
    }
