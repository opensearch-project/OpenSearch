...
- **Planner phase** (coordinator): identifies Lucene-eligible predicates, converts RexNode → QueryBuilder → serialized bytes via `LuceneFragmentConvertor` / `LuceneFilterExecutor.convertFragment`. No shard context needed.
- **Executor phase** (data node): receives serialized bytes, initializes with `DirectoryReader` + `QueryShardContext`, executes Lucene queries via `LuceneFilterExecutor`, returns BitSet. This is `LuceneSearchExecEngine.prepare` / `execute` / `close`.

Both phases use `LuceneFilterExecutor` but through different methods:

| Phase    | Role                                          | Method              | Runs On     | Needs Shard Context |
|----------|-----------------------------------------------|---------------------|-------------|---------------------|
...
This is still a valid Calcite plan — `LUCENE_DELEGATED` is a custom `SqlOperator` registered in the framework. The plan can be serialized, transported, and explained like any Calcite plan. The `byte[]` payload is the output of `executor.convertFragment`.
...
1. Initialize Lucene bridge:
   executor.initialize(directoryReader, queryShardContext)
...
2. DataFusion processes Parquet DocValues segment-by-segment.
   ...
   Segment 0 (maxDoc=5000):
     Batch 0: startDocId=0, endDocId=1024
       executor.executeForSegment(termQB_bytes, segOrd=0, start=0, end=1024)
         → BitSet for docs 0-1023 in segment 0
       executor.executeForSegment(prefixQB_bytes, segOrd=0, start=0, end=1024)
         → BitSet for docs 0-1023 in segment 0
       Intersect → pass to DataFusion for status > 200 on this batch

     Batch 1: startDocId=1024, endDocId=2048
       executor.executeForSegment(termQB_bytes, segOrd=0, start=1024, end=2048)
       ...

   Segment 1 (maxDoc=3000):
     Batch 0: startDocId=0, endDocId=1024
       executor.executeForSegment(termQB_bytes, segOrd=1, start=0, end=1024)
       ...

3. Helper methods for DataFusion to drive iteration:
   executor.getSegmentCount() → number of segments
   executor.getSegmentMaxDoc(segOrd) → maxDoc for batch boundary calculation
...
│  │                                                                   │  │
│  │  executor.convertFragment(LogicalFilter wrapping EQUALS)      │  │
│  │    │                                                              │  │
│  │    ▼                                                              │  │
│  │  ┌─────────────────────────────────────────────────────────────┐  │  │
│  │  │ LuceneFilterExecutor.convertFragment (PLANNER)                 │  │  │
│  │  │                                                             │  │  │
...
│  │                                                                   │  │
│  │  executionCtx from upstream (carries Reader + Task)  │  │
│  │    │                                                              │  │
│  │    ▼                                                              │  │
│  │  executor.initialize(directoryReader, queryShardContext)                                │  │
│  │  ┌─────────────────────────────────────────────────────────────┐  │  │
│  │  │ LuceneFilterExecutor.initialize (EXECUTOR)                    │  │  │
│  │  │                                                             │  │  │
│  │  │  directoryReader from ExecutionContext.getReader(...)            │  │  │
...
│  │  executor.execute(byte[])  ← full shard, or:                 │  │
│  │  executor.executeForSegment(byte[], segOrd, start, end)      │  │
│  │  ┌─────────────────────────────────────────────────────────────┐  │  │
│  │  │ LuceneFilterExecutor.execute / executeForSegment              │  │  │
│  │  │                                                             │  │  │
...
│  │  executor.close()                                             │  │
│  │  ┌─────────────────────────────────────────────────────────────┐  │  │
│  │  │ LuceneFilterExecutor.close (EXECUTOR — releases resources)    │  │  │
│  │  │  directoryReader = null, allocator.close()                  │  │  │
│  │  └─────────────────────────────────────────────────────────────┘  │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                                                                         │
│  ┌────────────────┐  ┌──────────────────────┐                          │
...
| `ExecutionContext`                                  | Carries `Reader`, `SearchShardTask`, and table name through execution lifecycle.     |
| `EngineResultStream` / `EngineResultBatch`         | Result stream of record batches returned by engine execution.                        |
| `AnalyticsSearchBackendPlugin`                      | SPI: extends `SearchExecEngineProvider`. `name()`, `createSearchExecEngine(ctx)`, `supportedOperators()`, `filterCapabilities()`, `getFragmentConvertor()`. |
| `SearchExecEngineProvider`                          | Factory: `name()`, `createSearchExecEngine(ExecutionContext)`.                       |
| `FragmentConvertor`                                 | Converts RelNode fragments to backend-specific bytes: `convertScanFragment()`, `convertShuffleReadFragment()`, `appendShuffleWriter()`. |
| `OperatorCapability`                                | Enum: SCAN, FILTER, AGGREGATE, SORT, JOIN, WINDOW, PROJECT, COORDINATOR_REDUCE.     |
| `FilterCapability`                                  | Scoped filter capability: operator + field type + data format.                       |
...
`LuceneSearchEnginePlugin` is the single plugin class. It implements three SPI interfaces:

| SPI Interface                    | Purpose                                                    |
|----------------------------------|------------------------------------------------------------|
| `SearchBackEndPlugin`            | Reader management. Creates `LuceneReaderManager` for `DirectoryReader` lifecycle. |
| `AnalyticsSearchBackendPlugin`   | Analytics query execution. `createSearchExecEngine(ctx)`, `supportedOperators()`, `filterCapabilities()`, `getFragmentConvertor()`. |
| `DataFormatPlugin`               | Registers `LuceneDataFormat` declaring field type capabilities (keyword, text, numeric, etc.). |

#### Query Execution Infrastructure (data node)

These components handle shard-level Lucene query execution. `LuceneSearchExecEngine` orchestrates through `LuceneFilterExecutor`, which delegates segment-level scoring to `LuceneIndexFilterProvider`.

| Component                    | Owner    | Role                                                                                     |
|------------------------------|----------|------------------------------------------------------------------------------------------|
| `LuceneSearchExecEngine`    | Upstream | `SearchExecEngine` implementation. Takes `LuceneSearchContext` in constructor. `prepare`/`execute`/`close`. |
| `LuceneFilterExecutor`      | Ours     | Core execution. Planner: `convertFragment` (RexNode → byte[]). Executor: `initialize(reader, qsc)`/`execute`/`executeForSegment`/`close`. Delegates scoring to `LuceneIndexFilterProvider`. Packages results as Arrow BitVector. |
| `LuceneSearchContext`       | Upstream | Data-node context. Holds Lucene `Query`, `DirectoryReader`, `SearchShardTask`, `LuceneEngineSearcher`. |
| `LuceneEngineSearcher`      | Upstream | `EngineSearcher` implementation. Creates Weight from Query via `IndexSearcher`. |
| `LuceneFragmentConvertor`   | Upstream | `FragmentConvertor` implementation. MVP: serializes `MatchAllQueryBuilder`. Future: uses our `RexToQueryBuilderConverter`. |
| `LuceneDataFormat`          | Upstream | Declares Lucene field type capabilities (keyword, text, numeric, date, etc.). |
| `LuceneIndexFilterProvider`  | Upstream | Per-segment scoring: Weight → Scorer → `DocIdSetIterator` → `long[]` bitset. Manages collector lifecycle. |
| `LuceneIndexFilterContext`   | Upstream | Per-(query, reader) context. Caches `Weight`, exposes segment count/maxDoc. |
| `LuceneReaderManager`       | Upstream | Manages `DirectoryReader` per `CatalogSnapshot`. |
...
| Component          | Role                                                                                     |
|--------------------|------------------------------------------------------------------------------------------|
| `DatafusionSearchExecEngine` | `SearchExecEngine`. Substrait → native Rust execution → result stream. |
...
| `ExecutionContext`             | `Reader` → `DirectoryReader` via `LuceneDataFormat`          | Executor | `LuceneSearchExecEngine.prepare()`     |
| `QueryShardContext`            | `fieldMapper(name)` → `MappedFieldType`                     | Executor | `LuceneFilterExecutor.execute()` via `toQuery()` |
| `DefaultPlanExecutor`          | Lifecycle orchestration: `prepare` → `execute` → `close`    | Executor | Per shard                              |
...
The plugin handles predicates on non-numeric fields (keyword, text, ip) — fields that have Lucene inverted indices. Numeric fields (integer, long, float, double, date) are handled by DataFusion over Parquet doc values and are out of scope for this plugin.

### Scope: Inverted Index Predicates Only

Lucene's inverted index supports term lookup, prefix/wildcard matching, regex, and phrase matching on tokenized or keyword fields. These operations are not efficiently expressible over columnar Parquet storage — they require the inverted index data structure. That's the value Lucene adds to the analytics engine.

Numeric comparisons (`>`, `<`, `BETWEEN`), date ranges, and point queries are excluded — DataFusion handles these natively over Parquet doc values with vectorized execution, which is more efficient than Lucene's BKD tree for columnar scans.

### Predicate Table

| SQL Shape                    | Field Type    | Lucene QueryBuilder          | Calcite SqlKind     | Status      |
|------------------------------|---------------|------------------------------|---------------------|-------------|
| `field = 'value'`           | keyword, text | `TermQueryBuilder`           | `EQUALS`            | Implemented |
| `field LIKE 'prefix%'`      | keyword, text | `PrefixQueryBuilder`         | `LIKE`              | Implemented |
| `field LIKE '%pat%'`        | keyword, text | `WildcardQueryBuilder`       | `LIKE`              | Implemented |
| `field LIKE '_attern'`      | keyword, text | `WildcardQueryBuilder`       | `LIKE`              | Implemented |
| `A AND B` (both Lucene)     | keyword, text | `BoolQueryBuilder(must)`     | `AND`               | Planned     |
| `A OR B` (both Lucene)      | keyword, text | `BoolQueryBuilder(should)`   | `OR`                | Planned     |
| `NOT A`                     | keyword, text | `BoolQueryBuilder(mustNot)`  | `NOT`               | Planned     |
| `field IN ('a','b','c')`    | keyword, text | `TermsQueryBuilder`          | `IN`                | Planned     |
| `field RLIKE 'pattern'`     | keyword, text | `RegexpQueryBuilder`         | `SIMILAR_TO`        | Planned     |
| `field IS NULL`             | keyword, text | `ExistsQueryBuilder` (negated) | `IS_NULL`         | Planned     |
| `field IS NOT NULL`         | keyword, text | `ExistsQueryBuilder`         | `IS_NOT_NULL`       | Planned     |

### Unresolved RexNodes (Script Predicates)

Some predicates may arrive as unresolved RexNodes — expressions that Calcite cannot decompose into standard operators. These typically represent:
- Scripted fields or painless expressions in the query body
- User-defined functions that don't map to standard SQL operators
- Complex expressions that Calcite leaves as opaque `RexCall` nodes

These cannot be converted to Lucene QueryBuilder types because they have no equivalent inverted index operation. The plugin should reject them via `canHandle` returning false, letting DataFusion handle them as in-memory evaluation over scanned rows. If a scripted predicate operates on a text field and could benefit from Lucene (e.g., a script that does regex matching), a future `ScriptPredicateHandler` could convert it to a `ScriptQueryBuilder` — but this is out of scope for now.

### Not in Scope (Handled by DataFusion over Parquet)

| SQL Shape                    | Why Not Lucene                                              |
|------------------------------|-------------------------------------------------------------|
| `field > 100` (numeric)     | BKD tree less efficient than vectorized Parquet scan         |
| `field BETWEEN 10 AND 20`   | Point range query — Parquet handles natively                 |
| `date_field > '2024-01-01'` | Date range — Parquet doc values with vectorized comparison   |
| `field + 1 > 10`            | Arithmetic expression — no inverted index equivalent         |
| Aggregations                 | Lucene collectors exist but DataFusion's columnar aggs are faster |
...
| SearchExecEngine lifecycle          | `QueryShardContext`, `MapperService`, `NamedWriteable` |
...
Phase 3:        Aggregations. Lucene collectors via SearchExecEngine. Parallels AggregationPhase.
...
- **Single-threaded per executor instance.** `directoryReader` and `queryShardContext` are mutable fields with no synchronization. One executor per shard, single-threaded access.
...
- **`LUCENE_DELEGATED` operators are resolved by the executor, not DataFusion.** The executor walks the plan, executes all `LUCENE_DELEGATED` nodes via `executor.execute`, and replaces them with BitSet results before passing the plan to DataFusion. DataFusion never sees the custom operator — it only receives a pre-computed BitSet filter alongside its own predicates.
