...
import org.opensearch.plugin.iceberg.catalog.s3.async.AsyncTransferEventLoopGroup;
...
class S3AsyncService implements Closeable {
    ...
    S3AsyncService(final Path configPath, @Nullable ScheduledExecutorService clientExecutorService) {
        ...
        staticClientSettings = MapBuilder.<String, S3ClientSettings>newMapBuilder()
            .put(
                buildClientName("default", S3Settings.CRT_ASYNC_HTTP_CLIENT_TYPE),
                S3ClientSettings.getClientSettings(Settings.EMPTY, "default", configPath)
            )
            .put(
                buildClientName("default", S3Settings.NETTY_ASYNC_HTTP_CLIENT_TYPE),
                S3ClientSettings.getClientSettings(Settings.EMPTY, "default", configPath)
            )
            .immutableMap();
    ...
    public synchronized void refreshAndClearCache(Map<String, S3ClientSettings> clientsSettings) {
        ...
        for (Map.Entry<String, S3ClientSettings> entrySet : clientsSettings.entrySet()) {
            defaultBuilder.put(
                buildClientName(entrySet.getKey(), S3Settings.CRT_ASYNC_HTTP_CLIENT_TYPE),
                clientsSettings.get(entrySet.getKey())
            );
            defaultBuilder.put(
                buildClientName(entrySet.getKey(), S3Settings.NETTY_ASYNC_HTTP_CLIENT_TYPE),
                clientsSettings.get(entrySet.getKey())
            );
        }
        ...
        assert this.staticClientSettings.containsKey(buildClientName("default", S3Settings.NETTY_ASYNC_HTTP_CLIENT_TYPE))
            : "Static Client Settings should contain default Netty client";
        assert this.staticClientSettings.containsKey(buildClientName("default", S3Settings.CRT_ASYNC_HTTP_CLIENT_TYPE))
    ...
    public AmazonAsyncS3Reference client(
        ...
        String asyncHttpClientType = S3Settings.S3_ASYNC_HTTP_CLIENT_TYPE.get(repositoryMetadata.settings());
    ...
    S3ClientSettings settings(RepositoryMetadata repositoryMetadata) {
        ...
        final String clientName = buildClientName(
            S3Settings.CLIENT_NAME.get(settings),
            S3Settings.S3_ASYNC_HTTP_CLIENT_TYPE.get(repositoryMetadata.settings())
        );
    ...
    static SdkAsyncHttpClient buildHttpClient(
        ...
        if (S3Settings.NETTY_ASYNC_HTTP_CLIENT_TYPE.equals(asyncHttpClientType)) {
