...
public final class S3ClientSettings {
    ...
    static boolean checkDeprecatedCredentials(Settings repositorySettings) {
        if (S3Settings.ACCESS_KEY_SETTING.exists(repositorySettings)) {
            if (S3Settings.SECRET_KEY_SETTING.exists(repositorySettings) == false) {
                throw new IllegalArgumentException(
                    "Repository setting ["
                        + S3Settings.ACCESS_KEY_SETTING.getKey()
                        + " must be accompanied by setting ["
                        + S3Settings.SECRET_KEY_SETTING.getKey()
                        + "]"
                );
            }
            return true;
        } else if (S3Settings.SECRET_KEY_SETTING.exists(repositorySettings)) {
            throw new IllegalArgumentException(
                "Repository setting ["
                    + S3Settings.SECRET_KEY_SETTING.getKey()
                    + " must be accompanied by setting ["
    ...
    private static AwsCredentials loadDeprecatedCredentials(Settings repositorySettings) {
        ...
        try (
            SecureString key = S3Settings.ACCESS_KEY_SETTING.get(repositorySettings);
            SecureString secret = S3Settings.SECRET_KEY_SETTING.get(repositorySettings)
        ) {
