...
//    }
    ...
    public void testQueryThenFetchE2ETest() throws IOException, URISyntaxException, InterruptedException, ExecutionException {
        ...
        try {
            ...
            // Create a mock EngineSearcherSupplier that returns a DatafusionSearcher
            EngineSearcherSupplier<?> reader = mock(EngineSearcherSupplier.class);
            DatafusionSearcher datafusionSearcher = engine.acquireSearcher("search");
            when(reader.acquireSearcher(anyString())).thenReturn(datafusionSearcher);
            ...
            if (totalHits == 0) {
                logger.info("Query returned no results, test passes with empty result set");
                return;
            }
