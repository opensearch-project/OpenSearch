...
public class DatafusionSearcher implements EngineSearcher<DatafusionQuery, RecordBatchStream> {
    ...
    public void search(DatafusionQuery datafusionQuery, List<SearchResultsCollector<RecordBatchStream>> collectors) throws IOException {
        ...
        long nativeStreamPtr = DataFusionQueryJNI.executeSubstraitQuery(
            reader.getCachePtr(),
    ...
    @Override
    public long search(DatafusionQuery datafusionQuery, ReadEngineConfig config, Long runtimePtr) {
        return DataFusionQueryJNI.executeSubstraitQuery(
            reader.getCachePtr(),
            datafusionQuery.getSubstraitBytes(),
            config.getSessionConfig().getOrCreateNativePtr(),
            runtimePtr, datafusionQuery.getDataformat());
    }
