...
pub extern "system" fn Java_org_opensearch_datafusion_jni_NativeBridge_shutdownTokioRuntimeManager(
    ...
    rust_log_info!("Runtime manager shut down started");
    if let Some(mgr) = TOKIO_RUNTIME_MANAGER.get() {
        mgr.shutdown();
        rust_log_info!("Runtime manager shut down successfully");
    }
}
...
pub extern "system" fn Java_org_opensearch_datafusion_jni_NativeBridge_startTokioRuntimeMonitoring(
    ...
    let manager = match TOKIO_RUNTIME_MANAGER.get() {
        Some(m) => m,
        None => {
            rust_log_info!("Tokio runtime manager not initialized");
            return;
        }
    };
...
fn log_runtime_metrics(metrics: &tokio_metrics::RuntimeMetrics) {
    rust_log_info!("=== Runtime Metrics ===");
    rust_log_info!("  Workers: {}", metrics.workers_count);
    rust_log_info!("  Global queue depth: {}", metrics.global_queue_depth);
    /*
    //unstable tokio causes build failures, uncomment this when monitoring

    rust_log_info!("  Worker overflow: {}", metrics.total_overflow_count);
    rust_log_info!("  Remote schedule: {}", metrics.max_local_schedule_count);
    rust_log_info!("  Worker steal ops: {}", metrics.total_steal_operations);
    rust_log_info!("  Blocking queue depth: {}", metrics.blocking_queue_depth);
    rust_log_info!("  Max local queue depth: {}", metrics.max_local_queue_depth);
    rust_log_info!("  Min local queue depth: {}", metrics.min_local_queue_depth);
    rust_log_info!("  Max local schedule count: {}", metrics.max_local_schedule_count);
    rust_log_info!("  Min local schedule count: {}", metrics.min_local_schedule_count);
    rust_log_info!("  Queue depth: {}", metrics.total_local_queue_depth);
    rust_log_info!("  Total schedule count: {}", metrics.total_local_schedule_count);
    */
...
fn log_task_metrics(operation: &str, metrics: &tokio_metrics::TaskMetrics) {
    rust_log_info!("=== Task Metrics: {} ===", operation);
    rust_log_info!("  Scheduled duration: {:?}", metrics.total_scheduled_duration);
    rust_log_info!("  Poll duration: {:?}", metrics.total_poll_duration);
    rust_log_info!("  Idle duration: {:?}", metrics.total_idle_duration);
    rust_log_info!("  Mean poll duration: {:?}", metrics.mean_poll_duration());
    rust_log_info!("  Slow poll ratio: {:.2}%", metrics.slow_poll_ratio() * 100.0);
    rust_log_info!("  Mean first poll delay: {:?}", metrics.mean_first_poll_delay());
    rust_log_info!("  Total slow polls: {}", metrics.total_slow_poll_count);
    rust_log_info!("  Total long delays: {}", metrics.total_long_delay_count);
}
...
pub extern "system" fn Java_org_opensearch_datafusion_jni_NativeBridge_createGlobalRuntime(
    ...
    rust_log_info!("Spill Limit is being set to : {}", spill_limit);
...
pub extern "system" fn Java_org_opensearch_datafusion_jni_NativeBridge_executeQueryPhaseAsync(
    ...
    let manager = match TOKIO_RUNTIME_MANAGER.get() {
        Some(m) => m,
        None => {
            rust_log_info!("Runtime manager not initialized");
    ...
    let table_name: String = match env.get_string(&table_name) {
        Ok(s) => s.into(),
        Err(e) => {
            rust_log_error!("Failed to get table name: {}", e);
            set_action_listener_error(&mut env, listener,
    ...
    let plan_bytes_vec = match env.convert_byte_array(plan_bytes_obj) {
        Ok(bytes) => bytes,
        Err(e) => {
            rust_log_error!("Failed to convert plan bytes: {}", e);
            set_action_listener_error(&mut env, listener,
    ...
    let listener_ref = match env.new_global_ref(&listener) {
        Ok(r) => r,
        Err(e) => {
            rust_log_error!("Failed to create global ref: {}", e);
            set_action_listener_error(&mut env, listener,
    ...
    io_runtime.block_on(async move {
        ...
        match result {
            ...
            Err(e) => {
                with_jni_env(|env| {
                    rust_log_error!("Query execution failed: {}", e);
    ...
    io_runtime.block_on(async move {
        ...
        with_jni_env(|env| {
            match result {
                ...
                Err(err) => {
                    rust_log_error!("Stream next failed: {}", err);
...
pub extern "system" fn Java_org_opensearch_datafusion_jni_NativeBridge_executeFetchPhase(
    ...
    match env.get_long_array_region(values, 0, &mut row_ids[..]) {
        Ok(_) => {
            rust_log_debug!("Received array: {:?}", row_ids);
        }
        Err(e) => {
            let _ = env.throw_new(
                "java/lang/RuntimeException",
    ...
    let manager = match TOKIO_RUNTIME_MANAGER.get() {
        Some(m) => m,
        None => {
            rust_log_error!("Runtime manager not initialized");
