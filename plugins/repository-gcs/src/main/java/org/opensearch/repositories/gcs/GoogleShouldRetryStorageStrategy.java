...
public class GoogleShouldRetryStorageStrategy implements StorageRetryStrategy {
    ...
    private final StorageRetryStrategy storageRetryStrategy = StorageRetryStrategy.getLegacyStorageRetryStrategy();

    private final DelagateResultRetryAlgorithm<?> idempotentHandler = new DelagateResultRetryAlgorithm<>(
        storageRetryStrategy.getIdempotentHandler()
    );

    private final DelagateResultRetryAlgorithm<?> nonIdempotentHandler = new DelagateResultRetryAlgorithm<>(
        storageRetryStrategy.getNonidempotentHandler()
    );
