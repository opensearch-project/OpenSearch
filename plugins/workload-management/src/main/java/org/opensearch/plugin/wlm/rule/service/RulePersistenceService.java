...
public class RulePersistenceService {
    ...
    public static final int MAX_RETURN_SIZE_ALLOWED_PER_GET_REQUEST = 100;
    ...
    private void fetchRuleById(String id, ActionListener<GetRuleResponse> listener) {
        ...
        client.prepareGet(RULES_INDEX, id).execute(ActionListener.wrap(getResponse -> {
    ...
    private void fetchAllRules(Map<Attribute, Set<String>> attributeFilters, String searchAfter, ActionListener<GetRuleResponse> listener) {
        ...
        searchRequest.execute(ActionListener.wrap(searchResponse -> {
            try (ThreadContext.StoredContext context = storedContext) {
                listener.onResponse(handleGetAllRuleResponse(searchResponse));
            }
        }, e -> {
            try (ThreadContext.StoredContext context = storedContext) {
     ...
     */
    GetRuleResponse handleGetAllRuleResponse(SearchResponse searchResponse) {
        List<SearchHit> hits = Arrays.asList(searchResponse.getHits().getHits());
        Map<String, Rule<QueryGroupFeatureType>> ruleMap = hits.stream().map(hit -> {
        ...
        return new GetRuleResponse(ruleMap, nextSearchAfter, RestStatus.OK);
    }
