...
public class PerFieldMappingPostingFormatCodecTests extends OpenSearchTestCase {
    ...
    public void testFuzzySetEnabled() {
        Settings settings = Settings.builder().put(IndexMetadata.SETTING_VERSION_CREATED, org.opensearch.Version.CURRENT).put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, 1).put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0).put("index.optimize_doc_id_lookup.fuzzy_set.enabled", true).put("index.optimize_doc_id_lookup.fuzzy_set.false_positive_probability", 0.05).build();
        ...
        PerFieldMappingPostingFormatCodec codec = new PerFieldMappingPostingFormatCodec(Lucene101Codec.Mode.BEST_SPEED, mapperService, logger);
    ...
    public void testFuzzySetDisabled() {
        Settings settings = Settings.builder().put(IndexMetadata.SETTING_VERSION_CREATED, org.opensearch.Version.CURRENT).put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, 1).put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0).put("index.doc_id_fuzzy_set.enabled", false).build();
    ...
    public void testFuzzyFilterPostingsFormatIsUsedForDocId() {
        ...
        assertTrue("FuzzyFilterPostingsFormat should be used for _id when fuzzy set is enabled", format instanceof FuzzyFilterPostingsFormat);
    }
}
