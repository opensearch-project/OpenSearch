...
public class FlatObjectFieldTypeTests extends FieldTypeTestCase {
    ...
    private static MappedFieldType getFlatParentFieldType(
        ...
        flatObjectFieldType.setValueFieldType(builder.getKeywordFieldType(fieldName, VALUE_SUFFIX, flatObjectFieldType, vft));
        flatObjectFieldType.setValueAndPathFieldType(
            builder.getKeywordFieldType(fieldName, VALUE_AND_PATH_SUFFIX, flatObjectFieldType, vft)
        );
    ...
    public void testTermQuery() {
        ...
        MappedFieldType unsearchable = new FlatObjectFieldMapper.FlatObjectFieldType("field", null, false, true, Collections.emptyMap());
    ...
    public void testPrefixQuery() {
        // 1.test isSearchable=true, hasDocValues=true, mappedFieldTypeName=null
        {
            ...
            assertEquals(expected, ft.prefixQuery("foo", CONSTANT_SCORE_REWRITE, false, MOCK_QSC));
        }
        ...
        // 4.test isSearchable=false, hasDocValues=false, mappedFieldTypeName=null
        {
            ...
            IllegalArgumentException e = expectThrows(
                IllegalArgumentException.class,
                () -> ft.prefixQuery("foo", CONSTANT_SCORE_REWRITE, false, MOCK_QSC)
            );
            assertEquals(
                "Cannot search on field [field._value] since it is both not indexed, and does not have doc_values " + "enabled.",
                e.getMessage()
    ...
    public void testFuzzyQuery() {
        // 1.test isSearchable=true, hasDocValues=true, mappedFieldTypeName=null
        {
            ...
            assertEquals(expected, ft.fuzzyQuery("foo", Fuzziness.fromEdits(2), 1, 50, true, null, MOCK_QSC));
        }
    ...
    public void testRangeQuery() {
        // 1.test isSearchable=true, hasDocValues=true, mappedFieldTypeName=null
        {
            ...
            assertEquals(expected, ft.rangeQuery(new BytesRef("2"), new BytesRef("10"), true, true, MOCK_QSC));
        }
        ...
        // 4.test isSearchable=false, hasDocValues=false, mappedFieldTypeName=null
        {
            ...
            IllegalArgumentException e = expectThrows(
                IllegalArgumentException.class,
                () -> ft.rangeQuery(new BytesRef("2"), new BytesRef("10"), true, true, MOCK_QSC)
            );
            assertEquals(
                "Cannot search on field [field._value] since it is both not indexed, and does not have doc_values " + "enabled.",
                e.getMessage()
