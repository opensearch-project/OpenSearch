...
public class RemoteIndexShardCorruptionTests extends IndexShardTestCase {
    ...
    public void testLocalDirectoryContains() throws IOException {
        ...
        for (String file : localDirectory.listAll()) {
            ...
            if (corrupted == false) {
                assertTrue(indexShard.localDirectoryContainsFile(localDirectory, file, checksum));
            } else {
                assertFalse(indexShard.localDirectoryContainsFile(localDirectory, file, checksum));
