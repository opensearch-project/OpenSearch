...
import org.opensearch.common.geo.GeometryDocValue;
...
public class GeoShapeValueTests extends OpenSearchTestCase {
    ...
    public void testMissingGeoShapeValue() throws IOException {
        ...
        final GeometryDocValue[][] values = new GeometryDocValue[numDocs][];

        for (int i = 0; i < numDocs; ++i) {
            values[i] = new GeometryDocValue[1];
            int number = TestUtil.nextInt(random(), 1, 2);
            if (number == 1) {
                values[i][0] = GeometryDocValue.createGeometryDocValue(new Point(randomDouble() * 90, randomDouble() * 180));
            } else {
                values[i][0] = null;
            }
        ...
        final GeoShapeValue asGeoValues = new GeoShapeValue() {
            ...
            @Override
            public GeometryDocValue nextValue() {
                return values[doc][0];
            }
        ...
        final GeometryDocValue missingValue = GeometryDocValue.createGeometryDocValue(missing);
        for (int i = 0; i < numDocs; i++) {
