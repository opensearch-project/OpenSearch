...
public class CompositeIndexConfigSettingsTests extends OpenSearchTestCase {
    ...
    public void testInvalidMetricMappedKeywordType() {
        ...
        Map<String, MappedFieldType> fieldTypes = new HashMap<>();
        fieldTypes.put("dim1_field", new DateFieldMapper.DateFieldType("dim1_field"));
        fieldTypes.put("dim2_field", new DateFieldMapper.DateFieldType("dim2_field"));
        ...
        assertEquals(
            "dimension field [dim1_field] with field type [date] is not same as the source field type for composite field [my_field]",
            exception.getMessage()
        );
        // reset FeatureFlags to defaults
        FeatureFlags.initializeFeatureFlags(Settings.EMPTY);
    ...
    public void testDimTypeValidation() {
        ...
        assertEquals(
            "dimension field [dim2_field] with field type [date] is not same as the source field type for composite field [my_field]",
            ex.getMessage()
        );
    }
