...
public class FileSnapshotTests extends OpenSearchTestCase {
    ...
    public void testFileSnapshotPath() throws IOException {
        ...
        fileSnapshot = new FileSnapshot.TransferFileSnapshot(file, 12, null, null);

        assertFileSnapshotProperties(file);

        try (FileSnapshot sameFileSnapshot = new FileSnapshot.TransferFileSnapshot(file, 12, null, null)) {
            assertEquals(sameFileSnapshot, fileSnapshot);
        }

        try (FileSnapshot sameFileDiffPTSnapshot = new FileSnapshot.TransferFileSnapshot(file, 34, null, null)) {
