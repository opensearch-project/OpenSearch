...
public class TransportFieldCapabilitiesIndexActionTests extends OpenSearchSingleNodeTestCase {
    ...
    public void testFieldCapsDisableObjects() throws Exception {
        String mapping = XContentFactory.jsonBuilder()
            ...
            .startObject("attributes").field("type", "object").field("disable_objects", true).endObject()
        ...
        client().prepareIndex("test").setId("1")
            .setSource(XContentFactory.jsonBuilder()
                .startObject().startObject("attributes").startObject("foo").field("bar", "baz").endObject().endObject().endObject())
            .get();

        client().admin().indices().prepareRefresh("test").get();

        FieldCapabilitiesResponse response = client()
            .fieldCaps(new FieldCapabilitiesRequest().fields("*").indices("test"))
            .actionGet();
        ...
        // phantom intermediate path has no ObjectMapper and must not appear
        assertFalse("attributes.foo should not appear in field caps", fields.containsKey("attributes.foo"));
    }
    ...
    public void testFieldCapsDisableObjectsAfterUnrelatedDocument() throws Exception {
        ...
        client().prepareIndex("test").setId("2")
            .setSource("{\"key\": 1}", MediaTypeRegistry.JSON)
            .get();
        ...
        assertFalse("attributes.foo.foo.bar must not appear", fields.containsKey("attributes.foo.foo.bar"));
        assertTrue("expected attributes in field caps", fields.containsKey("attributes"));
    ...
    public void testFieldCapsNormalNestedObjectUnaffected() throws Exception {
        String mapping = XContentFactory.jsonBuilder()
            .startObject().startObject("properties")
            .startObject("user").field("type", "object").startObject("properties")
            .startObject("address").field("type", "object").startObject("properties")
        ...
        client().prepareIndex("test").setId("1")
            .setSource("{\"user\":{\"address\":{\"city\":\"Madrid\"}}}", MediaTypeRegistry.JSON)
            .get();
