...
public class RestShardsActionTests extends OpenSearchTestCase {
    ...
    private void assertTable(Table table) {
        ...
        assertThat(headers.get(90).value, equalTo("docs.deleted"));
        ...
        for (final List<Table.Cell> row : rows) {
            ...
            assertThat(row.get(88).value, equalTo(shardStats.getDataPath()));
            assertThat(row.get(89).value, equalTo(shardStats.getStatePath()));
            assertThat(row.get(90).value, equalTo(shardStats.getStats().getDocs().getDeleted()));
        }
    }
}
