...
public class RestShardsActionTests extends OpenSearchTestCase {
    ...
    private void assertTable(Table table) {
        ...
        assertThat(headers.get(84).value, equalTo("docs.deleted"));
        ...
        for (final List<Table.Cell> row : rows) {
            ...
            assertThat(row.get(82).value, equalTo(shardStats.getDataPath()));
            assertThat(row.get(83).value, equalTo(shardStats.getStatePath()));
            assertThat(row.get(84).value, equalTo(shardStats.getStats().getDocs().getDeleted()));
        }
    }
}
