...
/**
 * Integration tests for streaming search REST preview functionality.
 * This is a simplified test for the CI fix to ensure StreamSearchAction is resolvable.
 */
@OpenSearchIntegTestCase.ClusterScope(scope = OpenSearchIntegTestCase.Scope.SUITE, numDataNodes = 1)
public class StreamingPreviewIntegrationTests extends OpenSearchIntegTestCase {

    @Override
    protected Settings nodeSettings(int nodeOrdinal) {
        return Settings.builder().put(super.nodeSettings(nodeOrdinal)).put(STREAM_SEARCH_ENABLED.getKey(), true).build();
    }
    ...
    public void testStreamSearchActionResolvable() {
        ...
        assertTrue("Stream search should be enabled", clusterService().getClusterSettings().get(STREAM_SEARCH_ENABLED));
