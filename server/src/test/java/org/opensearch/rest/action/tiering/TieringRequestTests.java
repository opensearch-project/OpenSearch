...
public class TieringRequestTests extends OpenSearchTestCase {

    public void testTieringRequestWithListOfIndices() {
        TieringIndexRequest request = new TieringIndexRequest();
        request.indices("foo", "bar", "baz");
        request.tier(TieringIndexRequest.Tier.WARM);
    ...
    public void testTieringRequestWithNullOrNotSupportedTier() {
        TieringIndexRequest request = new TieringIndexRequest();
        request.indices("test");
        ActionRequestValidationException validationException = request.validate();
        assertNotNull(validationException);
        request.tier(TieringIndexRequest.Tier.HOT);
        validationException = request.validate();
        assertNotNull(validationException);
    }
    ...
    public void testSerDeOfTieringRequest() throws IOException {
        TieringIndexRequest request = new TieringIndexRequest("warm", "test");
    ...
    public void testTieringRequestEquals() {
        final TieringIndexRequest original = new TieringIndexRequest("warm", "test");
        original.indicesOptions(IndicesOptions.fromOptions(randomBoolean(), randomBoolean(), randomBoolean(), randomBoolean()));
        final TieringIndexRequest expected = new TieringIndexRequest("warm", original.indices());
