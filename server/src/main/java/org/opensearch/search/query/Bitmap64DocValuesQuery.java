...
/**
 * Filter with 64-bit bitmap for long fields using doc values.
 * <p>
 * Similar to Lucene SortedNumericDocValuesSetQuery but for 64-bit values.
 * The bitmap must be constructed with {@code new Roaring64NavigableMap(true)} (signed mode)
 * so that {@code first()} and {@code last()} return correct signed min/max bounds for the
 * range optimization in the two-phase iterator.
 *
 * @see Bitmap64IndexQuery
 */
