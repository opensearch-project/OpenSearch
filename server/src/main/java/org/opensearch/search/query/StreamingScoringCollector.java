...
public class StreamingScoringCollector implements Collector {
    ...
    private boolean shouldEmit() {
        ...
        // Debug logging
        System.out.println("shouldEmit check: docsCollected=" + docsCollected + 
                          ", enoughDocs=" + enoughDocs + 
                          ", timeElapsed=" + timeElapsed + 
                          ", minDocs=" + config.getMinDocsBeforeEmission() + 
                          ", interval=" + config.getEmissionIntervalMillis());
    ...
    private void tryEmit() {
        ...
        System.out.println("tryEmit: topDocs=" + topDocs + 
                          ", scoreDocs=" + (topDocs != null ? topDocs.scoreDocs.length : "null"));
        ...
        if (topDocs == null || topDocs.scoreDocs.length == 0) {
            ...
            TopDocs mockTopDocs = new TopDocs(new org.apache.lucene.search.TotalHits(docsCollected, org.apache.lucene.search.TotalHits.Relation.EQUAL_TO), mockScoreDocs);
    ...
    private BoundProvider.SearchContext createSearchContext() {
        ...
        return new BoundProvider.SearchContext() {
            @Override
            public int getDocCount() { return docsCollected; }
            
            @Override
            public TopDocs getTopDocs() { return topDocs; }
            
            @Override
            public float getKthScore() { return finalKthScore; }
            
            @Override
            public float getMaxPossibleScore() { return finalMaxPossibleScore; }
            
            @Override
            public String getModality() { return "text"; }
        };
    }
    ...
    public static class StreamingFrame {
        ...
        public BoundProvider.SearchPhase getPhase() { return phase; }
        public double getProgress() { return progress; }
        public double getBound() { return bound; }
        
        @Override
        public String toString() {
            return "StreamingFrame{seq=" + sequenceNumber + ", docs=" + docCount + 
                   ", stable=" + isStable + ", phase=" + phase + "}";
        }
    }
}
