...
public interface SystemGeneratedProcessor extends Processor {
    /**
     * @return when to execute the processor. If it is null then default to be executed post user defined processors.
     */
    ...
    interface SystemGeneratedFactory<T extends Processor> extends Factory<T> {
        /**
         * Since we will invoke this function to every search request it should not be a time-consuming operation. We
         * should only build some simple if else logic here.
