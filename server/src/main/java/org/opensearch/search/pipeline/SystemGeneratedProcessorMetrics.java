...
class SystemGeneratedProcessorMetrics {
    private final Map<Processor.ProcessorType, Map<String, FactoryMetrics>> factoryMetrics = new HashMap<>();
    private final Map<Processor.ProcessorType, Map<String, OperationMetrics>> processorMetrics = new HashMap<>();

    SystemGeneratedProcessorMetrics() {
        for (Processor.ProcessorType type : Processor.ProcessorType.values()) {
            factoryMetrics.put(type, new HashMap<>());
            processorMetrics.put(type, new HashMap<>());
        }
    }
