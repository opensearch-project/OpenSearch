...
public interface SearchAnalyticsBackEndPlugin {
    ...
    /**
     * Creates an {@link IndexFilterTreeProvider} for boolean tree queries.
     * The tree provider composes multiple {@link IndexFilterProvider} instances,
     * one per collector leaf in the tree.
     *
     * @param format    the data format
     * @param shardPath the shard path
     * @return the tree provider, or null if not supported
     */
    default IndexFilterTreeProvider<?, ?, ?> createIndexFilterTreeProvider(DataFormat format, ShardPath shardPath) throws IOException {
        return null;
    }
}
