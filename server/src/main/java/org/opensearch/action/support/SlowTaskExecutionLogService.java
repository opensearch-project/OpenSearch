...
public class SlowTaskExecutionLogService {
    ...
    public static final Setting<TaskSlowLogLevel> SEARCH_TASK_SLOWLOG_LEVEL_SETTING =
        new Setting<>(SEARCH_TASK_SLOWLOG_PREFIX + ".level", TaskSlowLogLevel.TRACE.name(), TaskSlowLogLevel::parse, Setting.Property.Dynamic,
    ...
    public void maybeLog(Task task, long tookInNanos) {
        if(Regex.simpleMatch(searchTaskRegex, task.getAction())) {
            ...
            } else if (searchTaskInfoThreshold >= 0 && tookInNanos > searchTaskInfoThreshold && level.isLevelEnabledFor(TaskSlowLogLevel.INFO)) {
                searchSlowTaskLogger.info(new SearchTaskSlowLogMessage(task, tookInNanos));
            } else if (searchTaskDebugThreshold >= 0 && tookInNanos > searchTaskDebugThreshold && level.isLevelEnabledFor(TaskSlowLogLevel.DEBUG)) {
                searchSlowTaskLogger.debug(new SearchTaskSlowLogMessage(task, tookInNanos));
            } else if (searchTaskTraceThreshold >= 0 && tookInNanos > searchTaskTraceThreshold && level.isLevelEnabledFor(TaskSlowLogLevel.TRACE)) {
