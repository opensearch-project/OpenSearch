...
public final class RemoteStoreRefreshListener extends ReleasableRetryableRefreshListener {
    ...
    public static boolean isFileExcluded(String file) {
        ...
        return "write.lock".equals(file) || file.startsWith("segments_");
    }
                    ...
                    ActionListener<Void> segmentUploadsCompletedListener = new LatchedActionListener<>(new ActionListener<>() {
                        ...
                        public void onResponse(Void unused) {
                            try {
                                ...
                                var lastRemoteSegmentGeneration = Math.max(
                                    indexShard.getLastRemoteSegmentGeneration(),
                                    SegmentInfos.getLastCommitGeneration(localSegmentsPostRefresh.toArray(new String[0]))
                                );

                                indexShard.setLastRemoteSegmentGeneration(lastRemoteSegmentGeneration);
    ...
    private boolean isRefreshAfterCommit() throws IOException {
        ...
        if (indexShard.getLastRemoteSegmentGeneration() == -1) {
            try {
                RemoteSegmentMetadata remoteMetadata = remoteDirectory.readLatestMetadataFile();
                if (remoteMetadata == null) {
                    return true;
                }
                ReplicationCheckpoint checkpoint = remoteMetadata.getReplicationCheckpoint();
                return localCommitGeneration > checkpoint.getSegmentsGen();
            } catch (Exception e) {
                logger.debug("Exception occurred while fetching remote metadata file", e);
                return true;
            }
        }

        return localCommitGeneration > indexShard.getLastRemoteSegmentGeneration();
    }
