...
public final class KeywordFieldMapper extends ParametrizedFieldMapper {
    ...
    public static class KeywordFieldType extends StringFieldType {
        ...
        public Query regexpQuery(
            ...
            if (hasDocValues()) {
                String dvPattern = (String) rewriteForDocValue(value);
                CompiledAutomaton compiled = RegexpAutomatonCache.getInstance()
                    .getCompiledAutomaton(dvPattern, syntaxFlags, matchFlags, maxDeterminizedStates, RegexpQuery.DEFAULT_PROVIDER);
                return new PrecompiledAutomatonQuery(
                    new Term(name(), indexedValueForSearch(dvPattern)),
                    compiled,
                    dvPattern,
                    MultiTermQuery.DOC_VALUES_REWRITE
                );
