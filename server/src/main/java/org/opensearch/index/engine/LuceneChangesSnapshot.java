...
final class LuceneChangesSnapshot implements Translog.Snapshot {
    ...
    private static final String FALLBACK_NO_OP_REASON = "reason unavailable because stored _source is missing";
    ...
    private Translog.Operation readDocAsOp(int docIndex) throws IOException {
        ...
        if (isTombstone && fields.id() == null) {
            ...
            // Unlike index operations, a no-op has no document source to replay; _source stores only the reason string.
            // Preserve the sequence number even if that diagnostic payload is missing.
            op = new Translog.NoOp(seqNo, primaryTerm, source == null ? FALLBACK_NO_OP_REASON : source.utf8ToString());
    ...
    private Translog.Operation handleMissingSource(long seqNo) {
