...
public interface DocumentInput<T> extends AutoCloseable {
    ...
    /**
     * Signals that a new element of a {@code nested} object begins.
     *
     * <p>Document parsing emits this once per nested array element (and once for a single nested
     * object value), before any of the element's fields arrive through {@link #addField}. Nested
     * objects declared inside another nested object's scope produce properly nested start/end
     * pairs. Every call is matched by exactly one {@link #endNestedElement()}, even when parsing
     * fails midway through the element.
     *
     * <p>Formats without a per-element representation of nested data can ignore both signals; the
     * default implementations are no-ops.
     *
     * @param path the full dotted path of the nested object this element belongs to
     */
    default void startNestedElement(String path) {}

    /**
     * Signals that the nested element opened by the matching {@link #startNestedElement(String)}
     * ends. Fields arriving after this call belong to the enclosing scope. The default
