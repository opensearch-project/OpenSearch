...
public class IntValuesComparatorSource extends IndexFieldData.XFieldComparatorSource {
    ...
    private NumericDocValues getNumericDocValues(LeafReaderContext context, int missingValue) throws IOException {
        final SortedNumericDocValues values = getValues(context);
        if (nested == null) {
    ...
    public FieldComparator<?> newComparator(String fieldname, int numHits, boolean enableSkipping, boolean reversed) {
        ...
        final int dMissingValue = (Integer) missingObject(missingValue, reversed);
            ...
            public LeafFieldComparator getLeafComparator(LeafReaderContext context) throws IOException {
                return new IntLeafComparator(context) {
                    ...
                    protected NumericDocValues getNumericDocValues(LeafReaderContext context, String field) throws IOException {
                        return IntValuesComparatorSource.this.getNumericDocValues(context, dMissingValue);
                    }

                    @Override
                    public void setScorer(Scorable scorer) {
                        IntValuesComparatorSource.this.setScorer(scorer);
                    }
                };
            }
        };
    ...
    public BucketedSort newBucketedSort(
        ...
        return new BucketedSort.ForInt(bigArrays, sortOrder, format, bucketSize, extra) {
            private final int dMissingValue = (Integer) missingObject(missingValue, sortOrder == SortOrder.DESC);

            @Override
            public Leaf forLeaf(LeafReaderContext ctx) throws IOException {
                return new Leaf(ctx) {
                    private final NumericDocValues docValues = getNumericDocValues(ctx, dMissingValue);
