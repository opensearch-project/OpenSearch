...
public class Store extends AbstractIndexShardComponent implements Closeable, RefCounted {
    ...
    private void deleteFile(String fileName) {
        ...
        try {
            directory.deleteFile("SegRep cleanup post decref", fileName);
        } catch (IOException ex) {
            logger.error(() -> new ParameterizedMessage("failed to delete file [{}]", fileName), ex);
            throw new IllegalStateException("Can't delete " + fileName + " - cleanup failed", ex);
        }
    }
    ...
    public void decRefFileDeleter(Collection<String> files) {
        if (this.indexSettings.isSegRepEnabled()) {
            metadataLock.writeLock().lock();
            try (Lock writeLock = directory.obtainLock(IndexWriter.WRITE_LOCK_NAME)) {
                this.replicaFileTracker.decRef(files, this::deleteFile);
            } catch (IOException ex) {
