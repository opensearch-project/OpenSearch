...
public abstract class ProtobufRemoteConnectionStrategy implements ProtobufTransportConnectionListener, Closeable {
    ...
    enum ProtobufConnectionStrategy {
        SNIFF(
            ProtobufSniffConnectionStrategy.CHANNELS_PER_CONNECTION,
            ProtobufSniffConnectionStrategy::enablementSettings,
            ProtobufSniffConnectionStrategy::infoReader
        ) {
            @Override
        ...
        PROXY(
            ProtobufProxyConnectionStrategy.CHANNELS_PER_CONNECTION,
            ProtobufProxyConnectionStrategy::enablementSettings,
            ProtobufProxyConnectionStrategy::infoReader
        ) {
            @Override
            public String toString() {
                return "proxy";
        ...
        private final Supplier<ProtobufWriteable.Reader<ProtobufRemoteConnectionInfo.ModeInfo>> reader;

        ProtobufConnectionStrategy(
            int numberOfChannels,
            Supplier<Stream<Setting.AffixSetting<?>>> enablementSettings,
            Supplier<ProtobufWriteable.Reader<ProtobufRemoteConnectionInfo.ModeInfo>> reader
        ) {
        ...
        public ProtobufWriteable.Reader<ProtobufRemoteConnectionInfo.ModeInfo> getReader() {
            return reader.get();
        }
    }

    public static final Setting.AffixSetting<ProtobufConnectionStrategy> REMOTE_CONNECTION_MODE = Setting.affixKeySetting(
        "cluster.remote.",
        "mode",
        key -> new Setting<>(
            key,
            ProtobufConnectionStrategy.SNIFF.name(),
            value -> ProtobufConnectionStrategy.valueOf(value.toUpperCase(Locale.ROOT)),
    ...
    static ProtobufRemoteConnectionStrategy buildStrategy(
        ...
        switch (mode) {
            case SNIFF:
                return new ProtobufSniffConnectionStrategy(clusterAlias, transportService, connectionManager, settings);
            case PROXY:
                return new ProtobufProxyConnectionStrategy(clusterAlias, transportService, connectionManager, settings);
            default:
    ...
    static Set<String> getRemoteClusters(Settings settings) {
        final Stream<Setting.AffixSetting<?>> enablementSettings = Arrays.stream(ProtobufConnectionStrategy.values())
    ...
    public static boolean isConnectionEnabled(String clusterAlias, Settings settings) {
        ProtobufConnectionStrategy mode = REMOTE_CONNECTION_MODE.getConcreteSettingForNamespace(clusterAlias).get(settings);
        if (mode.equals(ProtobufConnectionStrategy.SNIFF)) {
            List<String> seeds = ProtobufSniffConnectionStrategy.REMOTE_CLUSTER_SEEDS.getConcreteSettingForNamespace(clusterAlias)
                .get(settings);
            return seeds.isEmpty() == false;
        } else {
            String address = ProtobufProxyConnectionStrategy.PROXY_ADDRESS.getConcreteSettingForNamespace(clusterAlias).get(settings);
    ...
    public static boolean isConnectionEnabled(String clusterAlias, Map<Setting<?>, Object> settings) {
        ProtobufConnectionStrategy mode = (ProtobufConnectionStrategy) settings.get(
            REMOTE_CONNECTION_MODE.getConcreteSettingForNamespace(clusterAlias)
        );
        if (mode.equals(ProtobufConnectionStrategy.SNIFF)) {
            List<String> seeds = (List<String>) settings.get(
                ProtobufSniffConnectionStrategy.REMOTE_CLUSTER_SEEDS.getConcreteSettingForNamespace(clusterAlias)
            );
            return seeds.isEmpty() == false;
        } else {
            String address = (String) settings.get(
                ProtobufProxyConnectionStrategy.PROXY_ADDRESS.getConcreteSettingForNamespace(clusterAlias)
            );
            return Strings.isEmpty(address) == false;
        }
    }
    ...
    */
    ...
    boolean shouldRebuildConnection(Settings newSettings) {
        ProtobufConnectionStrategy newMode = REMOTE_CONNECTION_MODE.getConcreteSettingForNamespace(clusterAlias).get(newSettings);
    ...
    protected abstract ProtobufRemoteConnectionInfo.ModeInfo getModeInfo();
    ...
    */
    static class StrategyValidator<T> implements Setting.Validator<T> {
        ...
        private final ProtobufConnectionStrategy expectedStrategy;
        private final String namespace;
        private final Consumer<T> valueChecker;

        StrategyValidator(String namespace, String key, ProtobufConnectionStrategy expectedStrategy) {
            this(namespace, key, expectedStrategy, (v) -> {});
        }

        StrategyValidator(String namespace, String key, ProtobufConnectionStrategy expectedStrategy, Consumer<T> valueChecker) {
        ...
        public void validate(T value, Map<Setting<?>, Object> settings, boolean isPresent) {
            Setting<ProtobufConnectionStrategy> concrete = REMOTE_CONNECTION_MODE.getConcreteSettingForNamespace(namespace);
            ProtobufConnectionStrategy modeType = (ProtobufConnectionStrategy) settings.get(concrete);
