...
* @opensearch.internal
*/
public class ProtobufThreadPool implements ProtobufReportingService<ProtobufThreadPoolInfo>, Scheduler {
    ...
    */
    ...
    private final ProtobufThreadPoolInfo threadPoolInfo;
    ...
    private final Map<String, ProtobufExecutorBuilder> builders;

    private final ScheduledThreadPoolExecutor scheduler;

    public Collection<ProtobufExecutorBuilder> builders() {
    ...
    public ProtobufThreadPool(final Settings settings, final ProtobufExecutorBuilder<?>... customBuilders) {
        this(settings, null, customBuilders);
    }

    public ProtobufThreadPool(
        final Settings settings,
        final AtomicReference<RunnableTaskExecutionListener> runnableTaskListener,
        final ProtobufExecutorBuilder<?>... customBuilders
    ) {
        assert Node.NODE_NAME_SETTING.exists(settings);

        final Map<String, ProtobufExecutorBuilder> builders = new HashMap<>();
        ...
        builders.put(
            Names.GENERIC,
            new ProtobufScalingExecutorBuilder(Names.GENERIC, 4, genericThreadPoolMax, TimeValue.timeValueSeconds(30))
        );
        builders.put(Names.WRITE, new ProtobufFixedExecutorBuilder(settings, Names.WRITE, allocatedProcessors, 10000));
        builders.put(Names.GET, new ProtobufFixedExecutorBuilder(settings, Names.GET, allocatedProcessors, 1000));
        builders.put(Names.ANALYZE, new ProtobufFixedExecutorBuilder(settings, Names.ANALYZE, 1, 16));
        builders.put(
            Names.SEARCH,
        ...
        builders.put(
            ...
            new ProtobufResizableExecutorBuilder(settings, Names.SEARCH_THROTTLED, 1, 100, runnableTaskListener)
        );
        builders.put(Names.MANAGEMENT, new ProtobufScalingExecutorBuilder(Names.MANAGEMENT, 1, 5, TimeValue.timeValueMinutes(5)));
        // no queue as this means clients will need to handle rejections on listener queue even if the operation succeeded
        // the assumption here is that the listeners should be very lightweight on the listeners side
        builders.put(Names.LISTENER, new ProtobufFixedExecutorBuilder(settings, Names.LISTENER, halfProcMaxAt10, -1, true));
        builders.put(Names.FLUSH, new ProtobufScalingExecutorBuilder(Names.FLUSH, 1, halfProcMaxAt5, TimeValue.timeValueMinutes(5)));
        builders.put(Names.REFRESH, new ProtobufScalingExecutorBuilder(Names.REFRESH, 1, halfProcMaxAt10, TimeValue.timeValueMinutes(5)));
        builders.put(Names.WARMER, new ProtobufScalingExecutorBuilder(Names.WARMER, 1, halfProcMaxAt5, TimeValue.timeValueMinutes(5)));
        builders.put(Names.SNAPSHOT, new ProtobufScalingExecutorBuilder(Names.SNAPSHOT, 1, halfProcMaxAt5, TimeValue.timeValueMinutes(5)));
        builders.put(
            Names.FETCH_SHARD_STARTED,
            new ProtobufScalingExecutorBuilder(Names.FETCH_SHARD_STARTED, 1, 2 * allocatedProcessors, TimeValue.timeValueMinutes(5))
        );
        builders.put(Names.FORCE_MERGE, new ProtobufFixedExecutorBuilder(settings, Names.FORCE_MERGE, 1, -1));
        builders.put(
            Names.FETCH_SHARD_STORE,
            new ProtobufScalingExecutorBuilder(Names.FETCH_SHARD_STORE, 1, 2 * allocatedProcessors, TimeValue.timeValueMinutes(5))
        );
        builders.put(Names.SYSTEM_READ, new ProtobufFixedExecutorBuilder(settings, Names.SYSTEM_READ, halfProcMaxAt5, 2000, false));
        builders.put(Names.SYSTEM_WRITE, new ProtobufFixedExecutorBuilder(settings, Names.SYSTEM_WRITE, halfProcMaxAt5, 1000, false));
        builders.put(
            Names.TRANSLOG_TRANSFER,
            new ProtobufScalingExecutorBuilder(Names.TRANSLOG_TRANSFER, 1, halfProcMaxAt10, TimeValue.timeValueMinutes(5))
        );
        builders.put(Names.TRANSLOG_SYNC, new ProtobufFixedExecutorBuilder(settings, Names.TRANSLOG_SYNC, allocatedProcessors * 4, 10000));
        builders.put(
            Names.REMOTE_PURGE,
            new ProtobufScalingExecutorBuilder(Names.REMOTE_PURGE, 1, halfProcMaxAt5, TimeValue.timeValueMinutes(5))
        );

        for (final ProtobufExecutorBuilder<?> builder : customBuilders) {
        ...
        for (final Map.Entry<String, ProtobufExecutorBuilder> entry : builders.entrySet()) {
            final ProtobufExecutorBuilder.ExecutorSettings executorSettings = entry.getValue().getSettings(settings);
        ...
        this.threadPoolInfo = new ProtobufThreadPoolInfo(infos);
    ...
    */
    ...
    @Override
    public ProtobufThreadPoolInfo info() {
    ...
    * @opensearch.internal
    */
    public static class Info implements ProtobufWriteable {
        ...
        private final ProtobufSizeValue queueSize;
        ...
        public Info(
            ...
            @Nullable ProtobufSizeValue queueSize
        ) {
            this.name = name;
            this.type = type;
            this.min = min;
        ...
        @Nullable
        public ProtobufSizeValue getQueueSize() {
