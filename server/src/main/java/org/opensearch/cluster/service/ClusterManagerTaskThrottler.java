 ...
 * If no setting is set, default threshold will be used based on task type for throttling when a task is registered with throttling enabled.
 */
public class ClusterManagerTaskThrottler implements TaskBatcherListener {
    ...
    protected ThrottlingKey registerClusterManagerTask(String taskKey, boolean throttlingEnabled) {
        ...
        if (throttlingEnabled && Objects.isNull(getThrottlingLimit(taskKey))) {
            tasksThreshold.put(taskKey, (long) ClusterManagerThrottlingDefaults.getDefaultThreshold(taskKey));
        }
    ...
    void validateSetting(final Settings settings) {
        ...
        for (String key : groups.keySet()) {
            ...
            int threshold = groups.get(key).getAsInt("value", ClusterManagerThrottlingDefaults.getDefaultThreshold(key));
    ...
    void updateSetting(final Settings newSettings) {
        ...
        for (String key : settingKeys) {
            ...
            int defaultThreshold = ClusterManagerThrottlingDefaults.getDefaultThreshold(key);
