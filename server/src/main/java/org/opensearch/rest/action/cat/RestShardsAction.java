...
public class RestShardsAction extends AbstractCatAction {
    ...
    public RestChannelConsumer doCatRequest(final RestRequest request, final NodeClient client) {
        ...
        clusterStateRequest.setCancelAfterTimeInterval(request.paramAsTime("cancel_after_time_interval", NO_TIMEOUT));
        parseDeprecatedMasterTimeoutParameter(clusterStateRequest, request, deprecationLogger, getName());
        clusterStateRequest.clear().nodes(true).routingTable(true).indices(indices);
        AdminAPIsUtility adminAPIsUtility = new AdminAPIsUtility();
        CancellableTask task = adminAPIsUtility.createAndCancelCancellableTask(client, clusterStateRequest.getCancelAfterTimeInterval());
        if (task != null) clusterStateRequest.setParentTask(client.getLocalNodeId(), task.getId());
        return channel -> client.admin().cluster().state(clusterStateRequest, new RestActionListener<ClusterStateResponse>(channel) {
            ...
            public void processResponse(final ClusterStateResponse clusterStateResponse) {
                ...
                if (task != null) indicesStatsRequest.setParentTask(client.getLocalNodeId(), task.getId());
