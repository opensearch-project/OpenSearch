...
public class RestViewAction {
    ...
    public static class SearchViewHandler extends BaseRestHandler {
        ...
        public List<Route> routes() {
            return List.of(
                new NamedRoute.Builder().path("/views/" + VIEW_ID_PARAMETER + "/_search").method(GET).uniqueName("cluster:views:search").build(),
                new NamedRoute.Builder().path("/views/" + VIEW_ID_PARAMETER + "/_search").method(POST).uniqueName("cluster:views:search").build()
            );
        }
