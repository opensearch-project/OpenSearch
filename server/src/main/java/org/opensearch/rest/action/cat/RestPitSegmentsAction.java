...
public class RestPitSegmentsAction extends AbstractCatAction {
    ...
    protected BaseRestHandler.RestChannelConsumer doCatRequest(final RestRequest request, final NodeClient client) {
        final String[] array = Strings.splitStringByCommaToArray(request.param("pit_id"));
        ...
        return channel -> client.admin().cluster().state(clusterStateRequest, new RestActionListener<ClusterStateResponse>(channel) {
            ...
            public void processResponse(final ClusterStateResponse clusterStateResponse) {
                ...
                pitSegmentsRequest.addPitId(array[0]);
                client.execute(PitSegmentsAction.INSTANCE, pitSegmentsRequest, new RestResponseListener<IndicesSegmentResponse>(channel) {
                    @Override
