...
public class PrimaryReplicaResyncIT extends OpenSearchIntegTestCase {
    ...
    public void testResyncTimeoutClearsStuckFlag() throws Exception {
        ...
        assertAcked(
            prepareCreate("test").setSettings(
                Settings.builder()
                    ...
                    // Stopping the primary node leaves the replica copy unassigned; without this the default
                    // 1m delayed allocation keeps the cluster yellow for longer than ensureGreen() waits.
        ...
        assertBusy(() -> {
            assertFalse("primaryReplicaResyncInProgress should be cleared by timeout watchdog", shard.isPrimaryReplicaResyncInProgress());
        }, 30, TimeUnit.SECONDS);

        // Clear the transport interception so future requests succeed
        futurePrimaryTransport.clearAllRules();

        // Now open allocation to nodeC and verify relocation succeeds. Drop to zero replicas first, otherwise
        // the replica copy orphaned by the stopped node immediately claims nodeC and blocks the primary move.
        assertAcked(
            client().admin()
                .indices()
    ...
    public void testAlreadyClosedExceptionClearsFlag() throws Exception {
        ...
        // Poll for the surviving copy to be promoted rather than sleeping for a fixed interval:
        // forbiddenApis rejects Thread.sleep, and a fixed wait is racy under load either way.
        assertBusy(() -> {
            ClusterState s = client().admin().cluster().prepareState().get().getState();
            ShardRouting newPrimary = s.routingTable().index("test").shard(0).primaryShard();
            assertNotNull(newPrimary);
