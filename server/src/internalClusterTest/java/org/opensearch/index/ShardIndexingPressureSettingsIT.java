...
public class ShardIndexingPressureSettingsIT extends OpenSearchIntegTestCase {
    ...
    public void testShardIndexingPressureFeatureEnabledDisabledSetting() {
        ...
        IndexingPressure primaryNodeLimits = internalCluster().getInstance(IndexingPressure.class, primaryName);
        IndexingPressure replicaNodeLimits = internalCluster().getInstance(IndexingPressure.class, replicaName);
        IndexingPressure coordinatingNodeLimits = internalCluster().getInstance(IndexingPressure.class, coordinatingOnlyNode);

        // Assert no tracking at shard level for indexing pressure since default setting is disabled
        ShardIndexingPressure shardIndexingPressure = internalCluster().getInstance(IndexingPressure.class, primaryName)
        ...
        assertEquals(bulkRequest.ramBytesUsed(), coordinatingNodeLimits.addAndGetTotalCoordinatingBytes(0));
        assertEquals(bulkShardRequestSize, primaryNodeLimits.addAndGetTotalPrimaryBytes(0));
        assertEquals(bulkShardRequestSize, replicaNodeLimits.addAndGetTotalReplicaBytes(0));
        ...
        assertEquals(bulkShardRequestSize, coordinatingShardTracker.totalCombinedCoordinatingAndPrimaryBytes.get());
        assertEquals(bulkShardRequestSize, coordinatingShardTracker.totalCoordinatingBytes.get());
        assertEquals(bulkShardRequestSize, primaryShardTracker.totalCombinedCoordinatingAndPrimaryBytes.get());
        assertEquals(bulkShardRequestSize, primaryShardTracker.totalPrimaryBytes.get());
        assertEquals(bulkShardRequestSize, replicaShardTracker.totalReplicaBytes.get());

        assertEquals(bulkRequest.ramBytesUsed() + bulkShardRequestSize, coordinatingNodeLimits.addAndGetTotalCoordinatingBytes(0));
        assertEquals(2 * bulkShardRequestSize, primaryNodeLimits.addAndGetTotalPrimaryBytes(0));
        assertEquals(2 * bulkShardRequestSize, replicaNodeLimits.addAndGetTotalReplicaBytes(0));
        ...
        assertEquals(2 * bulkRequest.ramBytesUsed() + bulkShardRequestSize,
            coordinatingNodeLimits.addAndGetTotalCoordinatingBytes(0));
        assertEquals(3 * bulkShardRequestSize, primaryNodeLimits.addAndGetTotalPrimaryBytes(0));
        assertEquals(3 * bulkShardRequestSize, replicaNodeLimits.addAndGetTotalReplicaBytes(0));
    }
    ...
    public void testShardIndexingPressureEnforcedEnabledDisabledSetting() throws Exception {
        ...
        assertEquals(1, coordinatingShardTracker.coordinatingLastSuccessfulRequestLimitsBreachedRejections.get());
        ...
        assertEquals(2, coordinatingShardTracker.coordinatingLastSuccessfulRequestLimitsBreachedRejections.get());
        ...
        assertEquals(2, coordinatingShardTracker.coordinatingRejections.get());
        assertEquals(3, coordinatingShardTracker.coordinatingLastSuccessfulRequestLimitsBreachedRejections.get());
    }
    ...
    public void testShardIndexingPressureVerifyShardMinLimitSettingUpdate() throws Exception {
        ...
        assertEquals(1000, coordinatingShardTracker.primaryAndCoordinatingLimits.get());
        assertEquals(1000, primaryShardTracker.primaryAndCoordinatingLimits.get());
        assertEquals(1500, replicaShardTracker.replicaLimits.get());
        ...
        primaryShardTracker = internalCluster().getInstance(IndexingPressure.class, primaryName)
                .getShardIndexingPressure().getShardIndexingPressureTracker(shardId);
        replicaShardTracker = internalCluster().getInstance(IndexingPressure.class, replicaName)
                .getShardIndexingPressure().getShardIndexingPressureTracker(shardId);
        coordinatingShardTracker = internalCluster().getInstance(IndexingPressure.class, coordinatingOnlyNode)
                .getShardIndexingPressure().getShardIndexingPressureTracker(shardId);

        // Verify updated initial shard limits
        assertEquals(10000, coordinatingShardTracker.primaryAndCoordinatingLimits.get());
        assertEquals(10000, primaryShardTracker.primaryAndCoordinatingLimits.get());
        assertEquals(15000, replicaShardTracker.replicaLimits.get());
    }
    ...
    public void testShardIndexingPressureLastSuccessfulSettingsUpdate() throws Exception {
        ...
        // This request breaches the threshold and hence will be rejected
        if (randomBoolean) {
            ShardIndexingPressureTracker coordinatingShardTracker = internalCluster()
                .getInstance(IndexingPressure.class, coordinatingOnlyNode)
                    .getShardIndexingPressure().getShardIndexingPressureTracker(shardId);
            expectThrows(OpenSearchRejectedExecutionException.class, () -> client(coordinatingOnlyNode).bulk(bulkRequest).actionGet());
            assertEquals(1, coordinatingShardTracker.coordinatingRejections.get());
            assertEquals(1, coordinatingShardTracker.coordinatingLastSuccessfulRequestLimitsBreachedRejections.get());
            ...
            assertEquals(1, primaryShardTracker.coordinatingRejections.get());
            assertEquals(1, primaryShardTracker.coordinatingLastSuccessfulRequestLimitsBreachedRejections.get());
        }
        ...
        if (randomBoolean) {
            ...
            ShardIndexingPressureTracker coordinatingShardTracker = internalCluster()
                .getInstance(IndexingPressure.class, coordinatingOnlyNode)
                    .getShardIndexingPressure().getShardIndexingPressureTracker(shardId);
            assertEquals(1, coordinatingShardTracker.coordinatingRejections.get());
            assertEquals(1, coordinatingShardTracker.coordinatingThroughputDegradationLimitsBreachedRejections.get());
        } else {
            successFuture = client(primaryName).bulk(bulkRequest);
            successFuture.actionGet();
            successFuture = client(primaryName).bulk(bulkRequest);
            ...
            assertEquals(1, primaryShardTracker.primaryThroughputDegradationLimitsBreachedRejections.get());
        }
    }
