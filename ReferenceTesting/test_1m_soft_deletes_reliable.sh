...
curl -s -X DELETE "$HOST/ecom_sd_1m" > /dev/null 2>&1 || true
curl -s -X PUT "$HOST/ecom_sd_1m" -H 'Content-Type: application/json' \
  -d @ReferenceTesting/ecommerce-field_mappings.json > /dev/null
echo "Created ecom_sd_1m"
...
with open('/tmp/test_sd_1m.ndjson', 'w') as f:
    for i in range(1000000):
        f.write(json.dumps({'index': {'_index': 'ecom_sd_1m', '_id': str(i)}}) + '\n')
...
"
TOTAL=$(wc -l < /tmp/test_sd_1m.ndjson)
OFF=1; COUNT=0; BATCH=20000
while [ $OFF -le $TOTAL ]; do
  END=$((OFF + BATCH - 1))
  sed -n "${OFF},${END}p" /tmp/test_sd_1m.ndjson > /tmp/_batch_sd.ndjson
...
curl -s -X POST "$HOST/ecom_sd_1m/_flush?force=true" > /dev/null
...
DEL_RESULT=$(curl -s -X POST "$HOST/ecom_sd_1m/_delete_by_query" -H 'Content-Type: application/json' -d '{
...
curl -s -X POST "$HOST/ecom_sd_1m/_refresh" > /dev/null
...
curl -s "$HOST/ecom_sd_1m/_segments" | python3 -c "
import sys, json
...
echo -n "  Doc count: "
curl -s "$HOST/ecom_sd_1m/_count" | python3 -c "import sys,json;print(json.load(sys.stdin)['count'])"
...
BEFORE=$(curl -s -X POST "$HOST/ecom_sd_1m/_search" -H 'Content-Type: application/json' -d "$QUERY")
...
RESULT=$(curl -s -X POST "$HOST/ecom_sd_1m/_star_tree/upgrade" -H 'Content-Type: application/json' -d '{
  "star_tree": {
    "name": "ecom_star_tree",
    "ordered_dimensions": [
...
AFTER=$(curl -s -X POST "$HOST/ecom_sd_1m/_search" -H 'Content-Type: application/json' -d "$QUERY")
