...
(* handover between terms, and one register captures both. The term-scoped  *)
...
VARIABLES
  ...
  (* for the ETag. Every successful write bumps it, so holding the         *)
  (* current token is what it means to own the chain.                      *)
