...
| **CAS chain** | The sequence of version tokens the fence passes through. Each successful conditional write produces the next one, so holding the current token *is* ownership. |
